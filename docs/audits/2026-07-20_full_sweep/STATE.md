...
2. Present the remediation plan to the user for approval, THEN execute in two themed batches:
   **R1 content batch** (all six builders + tex mirrors + regenerate docx/PDF/zips + one
   re-verify loop): FS-1, FS-7, FS-8, FS-9, FS-10(+Egashira note), FS-11 (needs Connor 1987
   bibliographic web-verify first), FS-12, FS-13, FS-14, FS-15, FS-19(doc half), FS-20, FS-21,
   FS-22, FS-23 (fix-or-waive per item), FS-25; optional polish lists (phase2b ×4 + phase8 ×4).
   **R2 lock/tooling batch**: FS-2..FS-6 (lock extensions), FS-11-lexicon, FS-16 (tex↔JS sync
   gate), FS-17 (builder __dirname paths), FS-18 (clone-aware gates), FS-19(lockfile);
   must-fire self-test for every new gate per D42.
3. FS-24 closure: adopt the audit-close no-loss standing rule as a §3 D-row.
4. Reconcile final state vs baseline 64e4f4f; tick T44 in §2; final commit.
Viva-prep sheet lives in phase8_results.md — surface it to the user at close.
