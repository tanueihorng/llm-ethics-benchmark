...
| V7 | P3 | `scripts/build_fyp_report_v5.js:1093` + 5 mirrors | Kharinaev entry omits the page range IEEE style requires for a journal article. | Crossref: `page` = **96771-96793**. | `IEEE Access, vol. 14, pp. 96771–96793, 2026, doi: …`. Update the four exact-string assertions in the lock. |
| V8 | P3 | `docs/RESULTS_CARD.md:63`, `docs/project_field_guide.html:412` | Mistral judge-vs-regex κ upper bound stated as 0.28. | `judge_agreement.json`: mistral_7b_base **0.2904**, 4bit 0.2544 → range **0.25–0.29**. Thesis Table 6.1 already has it right. | 0.25–0.28 → 0.25–0.29. |
| V9 | P3 | `docs/fyp-report-defense-deck-2026-07.html:113` | Card pairs "κ .68–.95" with "cross-checked **all 15 aliases**". Both statements are individually true, but juxtaposed they imply the range spans 15. | `judge_pairwise_agreement.json` has **10** aliases (κ .678–.954). Over all 15 (adding `_int8.json`: .645–.973) the range would be **.645–.973**. | "all ten base/NF4 aliases" (keeps .68–.95 true). |
...
| V15 | P3 | `scripts/build_fyp_thesis_v4.js:325` vs `build_fyp_report_v5.js:933` | The two texts of record disagree on the same hedge: the thesis asserts "both scorers' population agreement would sit higher"; the report says that direction "is not identified". | The thesis's parenthetical is the softer residue of the same κ-direction claim D47 removed. | Drop the parenthetical or adopt the report's wording. |
