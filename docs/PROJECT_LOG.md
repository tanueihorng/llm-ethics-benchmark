...
| **Last updated** | 2026-05-27 (23:30 UTC+8) |
...
| **Experiments** | **ALL 6 JOBS COMPLETE.** Qwen 2B: 60976 (10m06s) + 60977 (26m00s). Qwen 4B: 60978 (11m35s) + 60979 (24m44s). Llama 3B: 60980 (6m15s) + 60981 (13m09s). Results and pipeline labels (from `make analyze`): Qwen 2B ΔASR=−0.120/ΔOR=−0.016/ΔMMLU=−0.087 (capability_collapse_masquerading_as_safety). Qwen 4B ΔASR=−0.045/ΔOR=0.000/ΔMMLU=−0.004 (broad_degradation). Llama 3B ΔASR=+0.030/ΔOR=0.000/ΔMMLU=−0.043 (alignment_degradation). T10 complete. **NEXT: update report with bootstrap CI values → email Dr. Zhang (T1).** |
| **Current FYP report** | `docs/FYP_Report_2026-05-27.docx` (64276 bytes) — COMPLETE for all three pairs. Table 6.1 fully populated. Table 6.2 all-pair delta comparison with corrected pipeline labels. §6.6–§6.11 all pairs documented with analysis. §6.9 full RQ1–RQ5 synthesis. Updated Abstract, Ch7, Ch10. Bootstrap CIs from `results/analysis/` not yet incorporated (T12). |
...
- [x] ~~**T10. Run analysis.**~~ DONE 2026-05-27: Results SCPed from TC1; `make analyze` ran; `results/analysis/pair_interpretations.csv` confirms labels: qwen_2b=capability_collapse_masquerading_as_safety, qwen_4b=broad_degradation, llama_3_2_3b=alignment_degradation. Labels corrected throughout FYP report.
