...
| **Last updated** | 2026-05-24 (01:50 UTC+8) |
...
- [ ] **T2. Re-sync repo to TC1.** The May changes (configs/tc1.yaml offline env vars, scripts/prefetch_tc1.py, regenerated sbatch files, CLI argparse fix, generation.py enable_thinking=False, loader.py compute_dtype handling, updated tests) need to land at `/tc1home/FYP/utan001/fyp_quant/repo`. Either `git pull` if pushed, or `scp -r` from the Mac. Confirm by `diff`-ing one regenerated sbatch.
...
- [ ] **T4. One-time HF login on TC1 head node.** `huggingface-cli login` with a token whose HF account has accepted the Meta Llama 3.2 license. Required for Llama pair.
