...
| **Last updated** | 2026-05-27 (16:15 UTC+8) |
...
> **Session handoff (read this first if you're a fresh agent / new chat):** As of 2026-05-27 15:43 UTC+8, the TC1 environment is fully provisioned, current official Qwen3 + Llama weights and datasets are pre-cached, and the CUDA + qwen_2b_base/HarmBench smoke path has passed via sbatch. The very next step is **T8 — submit the 6-job full matrix** (`make cluster-submit CONFIG=configs/tc1.yaml JOBS_DIR=slurm/jobs_tc1`). See §2 for the full task list. The user is operating from `(fyp-tc1) [utan001@CCDS-TC1:0 ~/fyp_quant/repo]$` (TC1 head node) when stepping through cluster work, and from their Mac at `/Users/tanueihorng/fyp_quant` when editing code. Workflow loop is `git push` (Mac) → `git pull` (TC1).
...
| **TC1 access** | Approved (account `utan001`, partition `UGGPU-TC1`, QoS `normal`, window 03/2026–11/2026). `fyp-tc1` conda environment active. Repo on TC1 at commit `9b29de0` (`/tc1home/FYP/utan001/fyp_quant/repo`). Workflow: `git push` from Mac → `git pull` on TC1. |
