...
![Tests](https://img.shields.io/badge/tests-pytest-2ea44f?style=flat-square)
...
Across five matched pairs at HarmBench's **512-token reference budget** (the primary configuration), **no pair shows a statistically significant increase in genuine harmful compliance** under 4-bit — the only ΔASR whose CI excludes zero is Llama-3.2-3B's **−0.040, a decrease**, and no HarmBench ASR contrast survives a BH-FDR correction. The multiplicity-robust effects are all **capability losses** (and one over-refusal *decrease* that a pre-registered human audit finds **most plausibly a regex measurement artifact** — an independent 3-class refusal judge does not reproduce it, and that judge aligns far better with a blinded human annotator than the regex, strict κ 0.485 vs −0.006; T35+T36 / §6.12 Result 6. Under the **validation-informed parallel BH-FDR family** (over-refusal scored judge-strict, D51) **two contrasts survive, both capability** — and no scorer finds a statistically significant over-refusal increase). An apparent +0.055 increase in the smallest model at a shorter 128-token budget turned out to be a **truncation artefact** (60% of 128-token responses were provably cut off mid-generation) and dissolves to 0.000 at the reference budget.
...
| `llama_3_2_3b` | 0.100 → 0.060 | **−0.040** [−0.075, −0.010] | ✅ **yes (a decrease)** | capability_collapse_masq._as_safety *(directional)* |
| `mistral_7b` | 0.585 → 0.565 | −0.020 [−0.080, +0.040] | no | alignment_improvement *(directional)* |
...
> Multiplicity is handled explicitly: under a Benjamini–Hochberg FDR correction over the 20 primary contrasts, **zero HarmBench ASR contrasts survive**; the three survivors are capability/over-refusal effects (see report §6.5.1) — the two capability survivors rest on exact-match scoring, while the one over-refusal survivor is regex-scored and, per the pre-registered T36 human audit, most plausibly a regex measurement artifact; under the validation-informed parallel family (judge-strict over-refusal, D51) exactly two contrasts survive, both capability (§6.5.1). A full-repo scorer audit (PROJECT_LOG D36) confirmed every primary number is classifier-scored.
...
tests/               # live inventory: pytest --collect-only
```
