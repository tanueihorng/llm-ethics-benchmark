...
const ch1 = [
  ...
  ...numberedList([
    ...
    "A verification suite of 176 automated tests across twelve test files, covering schema validation, dataset loaders, the refusal parser (with comprehensive regression tests for the v2 expanded patterns), SLURM job generation, matrix reuse behaviour, analysis logic, v2 score-sidecar selection, and the judge-model validation layer (stub-backend tests, redaction enforcement, and an official-template regression check).",
  ]),
...
const ch5 = [
  ...
  buildTable(
    ...
    [
      ...
      ["pytest", "(installed via requirements.txt; 176 tests passing)"],
    ],
    [4200, 5160],
  ),
...
const appendixD = [
  ...
  PJ("The verification suite comprises 176 automated tests across twelve test files. The distribution is summarised in Table D.1; the test files are located under tests/ in the repository."),
  buildTable(
    ...
    [
      ...
      ["Total", "176", "All passing on the current commit."],
    ],
    [3000, 1200, 5160],
  ),
...
│   └── metrics/
├── tests/                    (176 tests across 12 files)
...
const appendixG = [
  ...
  buildTable(
    ...
    [
      ["2026-06-06 12:26", "FYP_Report_2026-05-27.docx (current)", "Pre-push judge audit hardening. The TC1 judge path remains fp16 by default, but the sbatch now uses a conservative batch size of 4 to leave activation/KV-cache headroom on the confirmed 32 GB V100. The judge loader uses low_cpu_mem_usage, and the runner's console banner reports the resolved backend model id instead of printing None when defaults are used. This is an operational robustness update; it does not change the scoring method, redaction contract, or pending-results status of §6.12."],
