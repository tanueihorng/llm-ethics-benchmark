...
const cover = [
  ...
  new Paragraph({
    alignment: AlignmentType.CENTER,
    children: [new TextRun({ text: "Revision: HarmBench ASR scored by the official HarmBench classifier (judge-primary, D16); v2 refusal regex retained as a secondary proxy", font: FONT, size: 20, italics: true, color: "555555" })],
  }),
...
const ch1 = [
  ...
  ...numberedList([
    ...
    "A verification suite of 178 automated tests across twelve test files, covering schema validation, dataset loaders, the refusal parser (with comprehensive regression tests for the v2 expanded patterns), SLURM job generation, matrix reuse behaviour, analysis logic, v2 score-sidecar selection, and the judge-model validation layer (stub-backend tests, redaction enforcement, and an official-template regression check).",
  ]),
...
const ch2 = [
  ...
  PJ("A body of recent work has argued that quantization is not behaviourally neutral. Studies across a wide range of model sizes have shown that smaller models can suffer severe degradation under aggressive four-bit quantization while larger models remain comparatively stable, and that instruction-following fidelity and hallucination rates can shift even when general benchmark scores appear preserved. Safety-specific studies make the point directly. Kharinaev et al. (arXiv:2502.15799, 2025) evaluate 66 quantized variants across multiple post-training and quantization-aware methods and find that quantization can degrade safety alignment, with no single method dominating across models and bit-widths. Egashira et al. (Exploiting LLM Quantization, NeurIPS 2024) show that widely used BitsAndBytes schemes — including NF4 — can be manipulated so that a benign full-precision model becomes harmful once quantized. HarmLevelBench (Belkhiter et al., arXiv:2411.06835, 2024) reports that the direction of the safety change under AWQ and GPTQ is non-uniform across attack types. Q-resafe (Chen et al., ICML 2025) confirms quantization-induced safety degradation and proposes a quantization-aware patching method to repair it. Importantly, most of these studies evaluate either perplexity or general capability, with safety considered (if at all) as a separate axis rather than jointly with capability against a fixed capability anchor — the gap this study targets."),
...
const ch3 = [
  ...
  buildTable(
    ...
    [
      ...
      ["over_refusal_regression", "‖ΔASR‖ < 0.02  AND  ΔOR ≥ +0.02  AND  ΔMMLU > −0.03", "Safety and capability held, but the model refuses materially more benign prompts — an over-refusal regression on its own axis, not a fallback degradation."],
...
const ch5 = [
  ...
  buildTable(
    ...
    [
      ...
      ["pytest", "(installed via requirements.txt; 178 tests passing)"],
    ],
    [4200, 5160],
  ),
...
const ch6 = [
  ...
  PJ("Table 6.1 records the per-pair benchmark results for all three model pairs together with bootstrap 95% confidence intervals on the deltas. HarmBench Attack Success Rate is reported under the official HarmBench classifier (the primary scorer; see §6.1.1 and §6.12), with the v2 refusal regex shown beneath each pair as a secondary non-refusal-rate proxy. XSTest over-refusal and MMLU accuracy are unchanged (the HarmBench classifier judges HarmBench behaviours only). Confidence intervals are computed by a paired bootstrap over the matched prompt set (2 000 resamples, seed 42); both members of every pair see the same prompts in the same order, so each bootstrap draw resamples prompt indices and recomputes the delta directly. A delta is marked statistically significant when its 95% CI excludes zero."),
  buildTable(
  ...
  PJ("Under the primary (judge) scorer, the statistically significant deltas are: Qwen 1.7B ΔASR (+0.055, CI [+0.010, +0.100]), Qwen 1.7B ΔMMLU (−0.087), and Llama 3.2 3B ΔMMLU (−0.043). The Qwen 1.7B ΔASR is the only HarmBench delta whose CI excludes zero under the official classifier, and it is positive — quantization increases genuine harmful compliance — alongside the significant capability drop, so the pair degrades on both axes (broad_degradation). The Qwen 4B ΔASR is positive but directional only (+0.025, CI touches zero). The Llama ΔASR is essentially zero. Note the contrast with the v2 proxy, which reports much higher absolute ASR and would have placed the only significant ΔASR on Qwen 4B rather than Qwen 1.7B — the divergence is analysed in §6.12. All XSTest over-refusal deltas are non-significant under both scorers, supporting the over-refusal null result for RQ2."),
  ...
  buildTable(
    ...
    [
      ["HarmBench ΔASR (judge)", "+0.055  ★", "+0.025", "0.000"],
      ["  HarmBench ΔASR (v2 proxy)", "−0.025", "+0.065", "0.000"],
      ["  HarmBench McNemar exact p", "0.027", "0.18", "1.00"],
      ["XSTest Δover-refusal", "−0.024", "−0.008", "+0.016"],
      ["MMLU Δaccuracy", "−0.087  ★", "−0.003", "−0.043  ★"],
      ["Interpretation label (judge-primary)", "broad_degradation", "alignment_degradation", "broad_degradation"],
      ["Evidence status (two-layer)", "confirmed", "directional", "confirmed"],
    ],
    [2400, 2600, 2600, 2700],
  ),
...
,
  ...
  PJ("With all six jobs evaluated and HarmBench ASR scored by the official classifier, it is now possible to give answers to all five research questions. Paired bootstrap 95% confidence intervals quantify the statistical uncertainty behind each answer. Three deltas are statistically significant under the primary scorers: Qwen 1.7B ΔASR (+0.055, judge), Qwen 1.7B ΔMMLU (−0.087), and Llama 3B ΔMMLU (−0.043). All other deltas — including the Qwen 4B and Llama ΔASR, all three ΔOR values, and Qwen 4B ΔMMLU — are within sampling noise. The Qwen 1.7B ΔASR is the only HarmBench result whose CI excludes zero in either direction; combined with its significant capability loss, the Qwen 1.7B broad_degradation is the single most consequential empirical result in the study."),
  ...
  buildTable(
    ...
    [
      ...
      ["llama_3_2_3b_base", "0.060", "0.040", "97.0%", "0.69"],
  ...
  PJ("Threats and scope. The judge is itself a model and can err; it is adopted because it is the field-standard reference classifier for HarmBench, run at full precision, with deterministic greedy decoding and zero parse failures on this dataset. It validates HarmBench only; XSTest over-refusal and MMLU are unaffected. A residual limitation is that no second independent judge (e.g. LlamaGuard or a frontier API classifier) was run to cross-check the HarmBench classifier itself; that is noted as future work (Chapter 9). The v2 regex is retained in the repository and in Table 6.1/6.2 as a transparent secondary proxy so that readers can see exactly how much, and where, refusal-based scoring diverges from genuine harmful-compliance scoring."),
];
...
const ch9 = [
  ...
  ...numberedList([
    ...
    "Second independent judge (cross-check the primary HarmBench classifier). Re-score the saved generations with a second judge that applies the same HarmBench rubric and compare the two judges' per-pair ΔASR and per-model agreement, to confirm the safety finding is not an artefact of one classifier. The infrastructure is now implemented as a derived validation layer (an OpenAI-compatible api_judge backend and scripts/judge_pairwise_agreement.py), with separate redacted scores.judge.api_judge.* sidecars and no modification of raw.jsonl, summary.json, or any existing sidecar; running it and folding in the result is the remaining step. (A frontier API judge with the same rubric is the most accurate cross-check; an open-weight guard model such as LlamaGuard is the most independent. Using an external API is a deliberate data-egress decision recorded in the project log.)",
...
const refs = [
  ...
  ...numberedList([
    ...
    "Kharinaev, A., Moskvoretskii, V., Shvetsov, E., et al. Investigating the Impact of Quantization Methods on the Safety and Reliability of Large Language Models. arXiv:2502.15799, 2025.",
    "Chen, K., Zhang, J., Hu, J., et al. Q-resafe: Assessing Safety Risks and Quantization-aware Safety Patching for Quantized Large Language Models. ICML 2025; arXiv:2506.20251, 2025.",
    "Egashira, K., Vero, M., Staab, R., He, J., Vechev, M. Exploiting LLM Quantization. NeurIPS 2024; arXiv:2405.18137, 2024.",
    "Belkhiter, Y., Zizzo, G., Maffeis, S. HarmLevelBench: Evaluating Harm-Level Compliance and the Impact of Quantization on Model Alignment. arXiv:2411.06835, 2024.",
  ]),
];
...
const appendixD = [
  ...
  PJ("The verification suite comprises 178 automated tests across twelve test files. The distribution is summarised in Table D.1; the test files are located under tests/ in the repository."),
  buildTable(
    ...
    [
      ...
      ["test_judge_validation.py", "13", "Judge-model validation: stub-backend scoring, sidecar redaction enforcement, ASR aggregation, raw-immutability, idempotency, the official-template byte-for-byte regression check, and judge-precision resolution (fp16/8bit/4bit + VRAM estimates)."],
      ...
      ["test_quant_analysis.py", "11", "Pairwise delta computation, interpretation labels, bootstrap CI logic, and v2 score-sidecar selection."],
      ...
      ["Total", "178", "All passing on the current commit."],
    ],
    [3000, 1200, 5160],
  ),
...
│   └── metrics/
├── tests/                    (178 tests across 12 files)
