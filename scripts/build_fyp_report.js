...
const ch1 = [
  ...
  ...numberedList([
    ...
    "A verification suite of 282 automated tests across twenty-two test files, covering schema validation, dataset loaders, the refusal parser (with comprehensive regression tests for the v2 expanded patterns), SLURM job generation, matrix reuse behaviour, analysis logic, v2 score-sidecar selection, and the judge-model validation layer (stub-backend tests, redaction enforcement, and an official-template regression check).",
  ]),
...
const ch5 = [
  ...
  buildTable(
    ...
    [
      ...
      ["pytest", "(installed via requirements.txt; 282 tests passing)"],
    ],
    [4200, 5160],
  ),
...
const ch6 = [
  ...
  PJ("Table 6.1 records the per-pair benchmark results for all five model pairs together with bootstrap 95% confidence intervals on the deltas. HarmBench Attack Success Rate is reported under the official HarmBench classifier (the primary scorer; see §6.1.1 and §6.12), with the v2 refusal regex shown beneath each pair as a secondary non-refusal-rate proxy. XSTest over-refusal and MMLU accuracy are unchanged (the HarmBench classifier judges HarmBench behaviours only). Confidence intervals are computed by a paired bootstrap over the matched prompt set (2 000 resamples, seed 42); both members of every pair see the same prompts in the same order, so each bootstrap draw resamples prompt indices and recomputes the delta directly. A delta is marked statistically significant when its 95% CI excludes zero."),
  buildTable(
...
const appendixD = [
  ...
  PJ("The verification suite comprises 282 automated tests across twenty-two test files. The distribution is summarised in Table D.1; the test files are located under tests/ in the repository."),
  buildTable(
    ...
    [
      ...
      ["test_judge_pairwise.py", "3", "API-judge yes/no/unparseable mapping (faked client), judge-vs-judge kappa, null-row skip."],
      ["test_sensitivity.py", "3", "Multi-seed sensitivity config + aggregator (summarise_deltas) and sbatch generator."],
      ...
      ["test_quant_int8.py", "16", "INT8 precision point (D34/T29): quant_method schema validator + baseline-consistency check, ModelSpec/build_model_spec threading, 8-bit detection in _quantization_active, the loader int8-vs-nf4 branch selection (sentinel configs, no GPU/bitsandbytes needed), and the configs/tc1_int8.yaml sweep config."],
      ["test_precision_sweep.py", "5", "fp16 -> INT8 -> NF4 precision-sweep analysis: per-metric deltas vs fp16 and graded/cliff/non-monotonic shape classification, plus graceful INT8-pending handling."],
      ["Total", "282", "Twenty-two files; all passing on the current commit."],
    ],
    [3000, 1200, 5160],
  ),
...
│   └── metrics/
├── tests/                    (282 tests across 22 files)
