...
const ch8 = [
  ...
  ...numberedList([
    ...
    "Partial capability proxy. The six-subject MMLU subset is a tractable but partial measure of general capability and does not include code generation, long-context reasoning, or tool-use evaluation. To begin addressing this, a second capability benchmark (ARC-Challenge) has been implemented as a plugin with the same exact-match scoring but a structurally different, reasoning-oriented question distribution; running it on the matched pairs will turn the capability anchor into a two-benchmark composite (run pending — see Chapter 9).",
...
const ch9 = [
  ...
  ...numberedList([
    ...
    "Second capability benchmark (run pending). The alignment-vs-capability disambiguation currently rests on a single 300-question MMLU subset. An ARC-Challenge plugin (allenai/ai2_arc, ~1,172 reasoning-oriented science questions, same exact-match scoring as MMLU) is now implemented and wired into the config, the analysis pipeline (which emits an ARC ΔACC with the same paired-bootstrap CI), and per-model TC1 sbatch; running it makes capability a two-benchmark composite and tests whether the MMLU-based capability claims (the −8.7 pp Qwen 1.7B and −4.3 pp Llama drops, and the Qwen 4B preservation that gates its label) replicate under a structurally different benchmark. ARC writes to results/<model>/arc/ and does not touch the existing artifacts.",
