...
const ch6 = [
  ...
  PJ("Experimental runs are in progress on the NTU TC1 GPU cluster. The Qwen 2B-class pair (Qwen3-1.7B baseline versus NF4 4-bit) completed successfully on 2026-05-27 (SLURM jobs 60976 and 60977). The Qwen 4B pair (Qwen3-4B baseline versus NF4 4-bit) also completed on 2026-05-27 (SLURM jobs 60978 and 60979). The Llama 3.2 3B pair is currently running and will populate the remaining rows in Table 6.1 upon completion. Bootstrap 95% confidence intervals and formal interpretation labels will be computed by the analysis pipeline (compare_quant_pairs.py) after all six model jobs finish. Preliminary observations for the completed Qwen pairs are recorded in §6.6 (Qwen 2B) and §6.8 (Qwen 4B)."),
  ...
  P("Table 6.1  Per-pair results. Qwen 2B complete (jobs 60976+60977). Qwen 4B complete (jobs 60978+60979). Llama pair pending. Bootstrap 95% CIs will be added once all pairs are complete.", { size: 18 }),
  ...
  buildTable(
    ["Metric", "qwen_2b  (Qwen3-1.7B → NF4)", "qwen_4b  (Qwen3-4B → NF4)"],
    [
      ["HarmBench ΔASR", "−0.120  (−15.5%)", "−0.045  (−5.2%)"],
      ["XSTest Δover-refusal", "−0.016  (−50.0%)", "0.000  (0.0%)"],
      ["MMLU Δaccuracy", "−0.087  (−14.0%)", "−0.004  (−0.5%)"],
      ["Interpretation label", "capability_collapse_masquerading_as_safety", "robust_preservation"],
    ],
    [2600, 3880, 3880],
  ),
  P("Table 6.2  Qwen family quantization deltas and interpretation labels. Absolute delta = 4-bit value − baseline value. Relative delta = absolute / baseline × 100%.", { size: 18 }),
  ...
  H3("RQ5 — Are effects consistent across model families? (pending)"),
  PJ("RQ5 requires the Llama 3.2 3B pair, which is currently running on TC1. Two directional predictions follow from the Qwen evidence. If size is the primary driver of sensitivity, the 3B Llama model might show intermediate behaviour — partial capability degradation, possibly consistent with a borderline capability-collapse or mild-degradation label. If the 2B Qwen pattern is family-specific — driven by Qwen's alignment recipe or architecture rather than by scale — the Llama 3B result might look more like the Qwen 4B robust-preservation pattern despite the lower parameter count. This analysis will be completed once the Llama pair jobs finish."),

  H3("Discussion: What the Qwen data tells us about safety evaluation under quantization"),
  PJ("The Qwen family results carry three implications that extend beyond the specific numbers. First, surface-level safety metrics can be misleading without a capability anchor. Both Qwen models show lower HarmBench ASR under quantization, but only the 4B model's reduction is potentially interpretable as alignment-related. Evaluating quantized models for deployment safety using only a harmful-compliance benchmark would classify both as safer than their baselines — which is correct for the 4B model but misleading for the 2B model, where the reduction reflects degraded instruction-following, not improved alignment. Second, the scale at which capability-collapse becomes a concern is model-family-dependent. The 1.7B Qwen model crosses a threshold where NF4 compression substantially degrades general instruction following; the 4B model does not. Deployment practitioners should treat these thresholds as empirical questions requiring measurement rather than assumptions. Third, over-refusal is not a significant quantization-induced risk in Qwen models at this scale range — both models become marginally more permissive on benign prompts under compression, if anything. This suggests that quantization-induced safety miscalibration, when it occurs, is more likely to manifest as reduced capability than as excessive caution."),
];
...
const ch10 = [
  ...
  PJ("The framework is complete and experimental results are substantially in hand. Four of the six model jobs have completed on the TC1 cluster, providing full Qwen family data. The Qwen 2B pair (Qwen3-1.7B baseline versus NF4 4-bit) produced a capability-collapse-masquerading-as-safety pattern: MMLU accuracy declined by 8.7 pp (−14.0% relative), while HarmBench ASR fell by 12.0 pp and XSTest over-refusal halved. This pattern shows that the apparent safety improvement for the 1.7B model is an artefact of general capability degradation. The Qwen 4B pair (Qwen3-4B baseline versus NF4 4-bit) produced robust preservation: MMLU accuracy is essentially unchanged (Δ = −0.4 pp, −0.5% relative), HarmBench ASR declined only 4.5 pp, and XSTest over-refusal is invariant. Together, these two pairs yield provisional answers to RQ1–RQ4: NF4 quantization does not increase harmful compliance or over-refusal in Qwen models; it significantly degrades capability in the 1.7B model but not the 4B model; and smaller Qwen models are substantially more sensitive to compression, with the capability-collapse threshold lying somewhere between 1.7B and 4B parameters for this model family and quantization method."),
  PJ("The Llama 3.2 3B pair is currently running on TC1. Once it completes, the analysis pipeline will assign formal interpretation labels and compute bootstrap confidence intervals across all three pairs, allowing RQ5 (cross-family consistency) to be addressed and all five research questions to be answered with a complete evidence base. The full pairwise delta analysis will establish a documented empirical baseline for future quantization-safety work in the compact-model regime."),
];
