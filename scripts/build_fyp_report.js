...
const ch1 = [
  ...
  ...numberedList([
    ...
    "A verification suite of 289 automated tests across twenty-three test files, covering schema validation, dataset loaders, the refusal parser (with comprehensive regression tests for the v2 expanded patterns), SLURM job generation, matrix reuse behaviour, analysis logic, v2 score-sidecar selection, and the judge-model validation layer (stub-backend tests, redaction enforcement, and an official-template regression check).",
  ]),
...
const ch5 = [
  ...
  buildTable(
    ...
    [
      ...
      ["pytest", "(installed via requirements.txt; 289 tests passing)"],
    ],
    [4200, 5160],
  ),
...
const appendixD = [
  ...
  PJ("The verification suite comprises 289 automated tests across twenty-three test files. The distribution is summarised in Table D.1; the test files are located under tests/ in the repository."),
  buildTable(
    ...
    [
      ...
      ["test_precision_sweep.py", "7", "fp16 -> INT8 -> NF4 precision-sweep analysis: per-metric deltas vs fp16, graded/cliff/non-monotonic shape classification, graceful INT8-pending handling, and the v2-proxy column preferring summary.v2.json with fallback to summary.json."],
      ...
      ["Total", "289", "Twenty-three files; all passing on the current commit."],
    ],
    [3000, 1200, 5160],
  ),
...
│   └── metrics/
├── tests/                    (289 tests across 22 files)
