...
const ch6 = [
  ...
  buildTable(
    ...
    [
      ...
      ["Interpretation label (judge-primary)", "broad_degradation", "alignment_degradation (directional)", "broad_degradation"],
    ],
    [2400, 2600, 2600, 2700],
  ),
  P("Table 6.2  All-pair quantization deltas and interpretation labels. Δ = 4-bit − baseline. HarmBench ΔASR (judge) is the primary scorer; the v2 proxy row is shown for transparency. ★ marks deltas whose paired-bootstrap 95% CI excludes zero. Under the judge, three deltas are significant: Qwen 1.7B ΔASR, Qwen 1.7B ΔMMLU, and Llama ΔMMLU. The Qwen 4B judge ΔASR (+0.025) is directional only — its CI touches zero — so the interpretation label is alignment_degradation by point estimate but not statistically confirmed. Labels are judge-primary (D16).", { size: 18 }),
...
const appendixH = [
  ...
  buildTable(
    ...
    [
      ...
      ["scores.judge.<judge>.jsonl", "Derived judge-model score sidecar; HarmBench classifier implementation is code-complete, TC1 run pending", "Keep separate from v2 sidecars; no prompt/response text; populate only from the judge-validation runner."],
      ["summary.judge.<judge>.json", "Derived judge-model aggregate summary; HarmBench classifier implementation is code-complete, TC1 run pending", "Use for judge-vs-v2 sensitivity reporting after the TC1 judge run; do not overwrite original summaries."],
    ],
    [1900, 3200, 4700],
  ),
  ...
  PJ("The next continuation point is operational, not architectural: T20 judge-model validation is implemented, but the HarmBench classifier has not yet been run on TC1. The code-complete path uses `ethical_benchmark/judges/validation.py`, `scripts/run_judge_validation.py`, `slurm/judge_validation.sbatch`, and `scripts/judge_agreement.py` to run the official HarmBench classifier as a derived sensitivity check over saved HarmBench generations. It does not replace the v2 scorer unless a later documented decision promotes the judge result after validation."),
  PJ("Operationally, a future TC1 session should first run `git pull --ff-only` in `/tc1home/FYP/utan001/fyp_quant/repo`, then cache the classifier on the head node with `python scripts/prefetch_tc1.py --config configs/tc1.yaml --judge`, then submit `sbatch slurm/judge_validation.sbatch`. After the job completes, copy `scores.judge.harmbench_cls.jsonl` and `summary.judge.harmbench_cls.json` sidecars back to the Mac, run `python scripts/judge_agreement.py`, and populate §6.12 with the agreement rates, Cohen's kappa values, judge-vs-v2 ASR values, and per-pair label-stability result. Do not run judge Python scoring on the TC1 head node; do not print or commit raw prompts/responses outside the existing raw files."),
];
