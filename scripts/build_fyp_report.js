...
const ch8 = [
  ...
  ...numberedList([
    ...
    "Partial capability proxy (now two-benchmark). The six-subject MMLU subset is a tractable but partial measure of general capability and does not include code generation, long-context reasoning, or tool-use evaluation. To address this, a second capability benchmark (ARC-Challenge, 1,172 reasoning-oriented questions) has now been run on all six models (§6.4.1). It confirms the direction of capability loss but shows the MMLU magnitudes are partly benchmark-specific — Qwen 1.7B's large MMLU drop does not replicate on ARC, while Qwen 4B shows a small significant ARC loss MMLU missed — so capability is now reported as a two-benchmark composite. The interpretation labels remain MMLU-anchored pending a formal composite-capability rule (Chapter 9).",
