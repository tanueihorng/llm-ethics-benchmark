...
const ch6 = [
  ...
  buildTable(
    ...
    [
      ...
      ["qwen_4b", "HarmBench ASR", "0.860", "0.815", "−0.045", "−5.2%"],
      ["qwen_4b", "XSTest over-refusal", "0.016", "0.016", "0.000", "0.0%"],
      ["qwen_4b", "MMLU accuracy", "0.747", "0.743", "−0.004", "−0.5%"],
      ["llama_3_2_3b", "HarmBench ASR", "0.950", "0.980", "+0.030", "+3.2%"],
      ["llama_3_2_3b", "XSTest over-refusal", "0.000", "0.000", "0.000", "0.0%"],
      ["llama_3_2_3b", "MMLU accuracy", "0.610", "0.567", "−0.043", "−7.0%"],
    ],
    [1600, 1900, 1700, 1700, 1230, 1230],
  ),
  P("Table 6.1  Complete per-pair results. All six jobs complete (TC1 SLURM jobs 60976–60981, 2026-05-27). Bootstrap 95% confidence intervals to be added by the analysis pipeline.", { size: 18 }),
  ...
  PJ("The headline finding for RQ5 is sign inconsistency on the ASR dimension. Both Qwen pairs produced a negative ΔASR (fewer harmful completions under quantization). The Llama 3.2 3B pair produced a positive ΔASR (+0.030, +3.2%): the 4-bit model complied with more harmful prompts than the baseline, not fewer. This reversal is the clearest evidence that NF4 quantization effects on harmful compliance are not cross-family consistent. The over-refusal dimension is the one consistent finding: no model family shows increased over-refusal under quantization. The capability dimension (MMLU) is directionally consistent (all pairs lose accuracy) but varies substantially in magnitude (−14.0% for Qwen 2B versus −0.5% for Qwen 4B versus −7.0% for Llama). See §6.11 for the full discussion."),
...
const ch7 = [
  ...
  PJ("The Qwen family results (§6.9) provide initial empirical grounding for the threats discussed in this chapter. The qwen_2b capability-collapse finding and the qwen_4b robust-preservation finding are used below as concrete examples where relevant."),
...
const ch10 = [
  ...
  PJ("All six experimental runs completed on the NTU TC1 GPU cluster on 2026-05-27 (SLURM jobs 60976–60981). The three completed pairs produce three distinct quantization profiles. The Qwen3-1.7B pair (capability_collapse_masquerading_as_safety): MMLU accuracy fell 8.7 pp and HarmBench ASR fell 12.0 pp simultaneously, revealing that the apparent safety improvement is a capability artefact — the model is less capable of complying with complex harmful instructions, not better aligned. The Qwen3-4B pair (alignment_improvement): ASR fell by 4.5 pp beyond the 0.02 harm tolerance while MMLU was preserved (−0.4 pp, within tolerance) and over-refusal was unchanged — a modest but genuine safety improvement at no measurable capability cost, the most desirable outcome in the taxonomy and direct evidence that NF4 compression need not damage alignment. The Llama 3.2 3B pair (alignment_degradation): ASR increased by 3.0 pp (exceeding the 0.02 harm threshold) and MMLU accuracy fell by 4.3 pp, representing the only pair where quantization directly worsens harmful compliance with a confirmed positive ΔASR. Critically, the Llama baseline ASR at full precision is 0.950 — 17.5 pp higher than the comparable Qwen model — showing that the alignment recipe is the dominant determinant of safety calibration and that quantization effects are superimposed on this much larger family-level difference."),
  PJ("Taken together, the results answer all five research questions. NF4 quantization does not universally increase harmful compliance (RQ1): the direction depends on the model family and baseline alignment strength. It does not increase over-refusal in any evaluated model (RQ2). It significantly degrades capability in smaller compact models but not larger ones (RQ3). Smaller Qwen models are substantially more sensitive than larger ones to NF4 compression (RQ4). And the effects are not cross-family consistent on the harmful compliance dimension, though over-refusal and capability direction are broadly consistent (RQ5). The framework and methodology are available for replication and extension to additional model families, scale points, and quantization methods."),
];
