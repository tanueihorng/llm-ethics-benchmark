...
const ch6 = [
  ...
  tbl(["Family", "Cohen’s κ (regex vs classifier)", "Reading"], [
    ...
    ["Mistral", "0.25 to 0.29", "worst; regex 0.890 vs judge 0.565 (4-bit)"],
...
const refs = [
  ...
  REF(19, "I. Proskurina, L. Brun, G. Metzler, and J. Velcin, \"When quantization affects confidence of large language models?,\" arXiv preprint arXiv:2405.00632, 2024."),
  REF(20, "H. Inan et al., \"Llama Guard: LLM-based input-output safeguard for human-AI conversations,\" Meta AI, 2023. arXiv:2312.06674."),
  REF(21, "J. Pineau et al., \"Improving reproducibility in machine learning research (a report from the NeurIPS 2019 reproducibility program),\" Journal of Machine Learning Research (JMLR), vol. 22, 2021. arXiv:2003.12206."),
  REF(22, "G. K. Sandve, A. Nekrutenko, J. Taylor, and E. Hovig, \"Ten simple rules for reproducible computational research,\" PLOS Computational Biology, vol. 9, no. 10, e1003285, 2013."),
  REF(23, "G. Wilson, J. Bryan, K. Cranston, J. Kitzes, L. Nederbragt, and T. K. Teal, \"Good enough practices in scientific computing,\" PLOS Computational Biology, vol. 13, no. 6, e1005510, 2017."),
  REF(24, "A. M. Smith et al., \"Journal of Open Source Software (JOSS): Design and first-year review,\" arXiv preprint arXiv:1707.02264, 2017."),
];

const appendix = [
  H1("Appendix A  Reproducibility Statement"),
  PJ("All code, configuration, and redacted result artefacts are in the project repository. The analysis reproduces byte-for-byte from the committed sidecars without a GPU; full experiments require the gated model weights and a CUDA GPU. The project maps to the ML Reproducibility Checklist [21]: models and algorithms are described (Chapters 3 to 4); datasets, splits, and sample counts are recorded in each run summary; code and declared dependencies are released; hyper-parameters and decoding controls are recorded in every summary; the compute infrastructure is recorded; and significance procedures are specified, with multiple comparisons disclosed. The dependency set is declared with minimum-version floors (a lockfile with exact pins is a future hardening step). The repository follows recognised research-software practice (scripts rather than manual steps, fixed seeds, explicit dependencies, and a documented project structure [22], [23]) and is packaged for reuse in line with open-source-software peer-review norms [24]. Raw generations are gitignored and hash-pinned; only redacted sidecars (identifiers, booleans, scalars) are released, so the study is reproducible without redistributing harmful text. The primary artefacts live under results_512/ (the 512-token study), with the 128-token run retained under results/ as the generation-length comparison; a checksum manifest pins 300 immutable raw files across both trees and the multi-seed arm."),
