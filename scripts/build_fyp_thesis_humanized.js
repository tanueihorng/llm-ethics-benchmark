...
const ch2 = [
  ...
  PJ("Post-training quantization compresses model weights to lower-precision formats. The bitsandbytes library provides two widely used schemes evaluated here: NF4, a four-bit \"normal-float\" format with double quantization [2], and LLM.int8(), an eight-bit mixed-precision scheme that keeps outlier dimensions in higher precision [1]. While the accuracy cost of these methods is well characterised, their effect on safety behaviour is contested. Empirical studies report mixed and method-dependent results: some find that quantization can degrade safety alignment, with effects that vary by method and attack type [4], [5]; a comprehensive multi-benchmark evaluation finds headline scores largely retained at four bits while cautioning that narrow evaluations can miss shifts on other behavioural dimensions [6]; and some show that quantization can be actively exploited to surface unsafe behaviour [7]. Closest to this thesis’s own headline, Hong et al. [8] scrutinise trustworthiness across five compression methods and eight trust dimensions and report that four-bit quantization broadly preserves trustworthiness (unlike pruning, which erodes it, and unlike more extreme three-bit compression, where degradation appears); it is the nearest prior analogue to the null-safety-at-four-bit picture found here, though that study covers larger models under a different trust taxonomy. This very inconsistency motivates a controlled, matched-pair design rather than cross-paper comparison, and it cautions against strong, uniform claims."),
...
const ch3 = [
  ...
  PJ("Four benchmarks span the two axes of interest. Harmful compliance (HarmBench [3]) and over-refusal (XSTest [9]) capture safety; MMLU [13] and ARC-Challenge [14] capture capability, using two independent benchmarks so that capability claims do not rest on a single test."),
  ...
  PJ("HarmBench Attack Success Rate is scored by the official HarmBench classifier (cais/HarmBench-Llama-2-13b-cls) [3] — prompted with the Hugging Face model-card template rather than the GitHub evaluation-harness variant, so absolute ASR levels are not directly harness-comparable while matched-pair deltas are unaffected — run in full precision so that the reference instrument is not itself confounded by quantization. A second, architecturally independent judge (GPT-4o; run 2026-06-14 — the API alias’s dated snapshot was not recorded, so the revision-pinned open-weight LlamaGuard third judge is the reproducible arm) re-scores the same generations as a robustness check [11], [12]; it applies a condensed one-sentence yes/no rubric that encodes the same harmful-behaviour criterion as the HarmBench classifier, not that classifier’s full enumerated template. A deterministic refusal regex is retained only as a secondary, transparent proxy. This choice is itself a research object: the thesis reports the agreement between scorers and shows that the regex over-counts, which is why it is demoted. MMLU and ARC use deterministic exact match; XSTest over-refusal uses the regex as its primary scorer, cross-checked by an independent three-class refusal judge as a sensitivity layer (§7.2) (the HarmBench classifier judges harmful compliance only). XSTest’s own protocol scores over-refusal with LLM-based three-way labelling (full compliance / partial refusal / full refusal); this study instead retains a deterministic binary refusal regex as the primary scorer, a deliberate, reproducibility-motivated deviation that yields a coarser refusal / non-refusal construct, and adds that three-way judge back only as a sensitivity cross-check."),
...
const ch5 = [
  ...
  tbl(["Parameter", "Value"], [
    ...
    ["Third judge", "Llama-Guard-3-8B (open-weight, revision-pinned)"],
...
const ch6 = [
  ...
  tbl(["Family", "Cohen’s κ (regex vs classifier)", "Reading"], [
    ["Qwen", "0.36 to 0.59", "fair to moderate; regex over-counts heavily (0.595 vs judge 0.255, 1.7B base)"],
    ["Mistral", "0.25 to 0.29", "fair, the lowest; regex 0.890 vs judge 0.565 (4-bit)"],
    ["Llama", "0.71 to 0.84", "substantial to almost perfect; little ambiguous middle ground"],
    ["Phi", "0.67 to 0.77", "substantial; like Llama"],
  ], [1900, 3660, 3800]),
  ...
  PJ("Over-refusal moves in the benign direction where it moves at all: the only over-refusal delta significant under the exact paired test is Phi-4-mini’s decrease (ΔOR = −0.048, CI [−0.076, −0.020]) under the primary regex scorer, and an independent refusal judge does not reproduce it (§7.2). A blinded human audit finds that judge substantially better aligned with the annotator than the regex, so that survivor is most plausibly a measurement artifact of the regex scorer rather than a validated over-refusal shift. Qwen3-1.7B shows a non-significant decrease (−0.024, McNemar p = 0.109). No pair becomes significantly more prone to over-refusal on benign prompts, under either scorer (RQ2). The Qwen3-1.7B pair, whose +0.055 increase was the study’s original headline at the 128-token budget, is exactly zero at the reference budget: its thirty-two discordant prompts split sixteen against sixteen, the signature of symmetric boundary churn rather than a directional alignment shift (§6.5)."),
  H2("6.3  The generation-budget artefact (why 512 tokens is the primary budget)"),
  PJ("The 128-token budget of the study’s first run truncated the majority of generations before completion: a direct prefix test shows 60.3 percent of the 2,000 paired responses were provably cut off (the 512-token generation continues past the point where the 128-token one stops), 30.5 percent stop naturally, and 9.2 percent diverge between the two nominally greedy runs (benign cross-run non-determinism, conservatively excluded, which makes 60.3 percent a floor). Truncation is family-heterogeneous (93.5 to 98.0 percent for Mistral, 3.0 to 4.0 percent for Llama), so the mechanism is family-specific even though the conclusion is not. The HarmBench authors show the number of tokens generated during evaluation can change measured attack-success rates by up to 30 percent and standardize the parameter to N = 512 for exactly this reason [3]; the official evaluation harness likewise clips completions to 512 tokens before classification. Re-running the full apparatus at that budget dissolves the apparent safety regression: Qwen3-1.7B’s ΔASR goes from +0.055 (then significant, McNemar p = 0.027) at 128 tokens to exactly 0.000 under the classifier (p = 1.000) at 512, with the second judge at +0.005 (also p = 1.000). A five-seed stochastic-decoding arm at the reference budget corroborates the null (per-seed ΔASR mean +0.013, range [0.000, +0.035], no seed individually significant, and the greedy estimate sits inside the range; all five seed deltas are non-negative, a sub-minimum-detectable-effect directional signal disclosed here). The arm has since been extended to all five pairs. Qwen3-4B’s per-seed ΔASR mean is +0.029, range [+0.010, +0.050] (1 of 5 seeds individually significant; greedy +0.040 inside the range); Llama’s is −0.024, range [−0.040, −0.010] (2 of 5 seeds individually significant, both decreases; greedy −0.040 inside the range); Mistral’s is +0.012, range [−0.005, +0.045] (0 of 5); and Phi’s is −0.012, range [−0.055, +0.015] (1 of 5). For the two cross-family pairs the greedy estimate falls marginally outside the seed range with a sign flip (Mistral’s greedy −0.020 just below its seed range, Phi’s greedy +0.020 just above its own), small and sign-unstable gaps (each 0.04 or less) around seed means that are themselves near zero (Mistral +0.012, Phi −0.012), not a directional signal the greedy run missed. This is not read off a single detection floor: per-pair minimum detectable effects vary (0.044 to 0.086 at n = 200), and one Phi seed is itself individually significant at −0.055 (McNemar p = 0.043), a decrease, and one of five, so it does not lift the pair-level reading. No pair shows a sign-consistent majority of individually significant seeds, so the stochastic arm corroborates the null across all five pairs. The capability deltas, by contrast, are essentially unchanged across budgets: truncation manufactured the safety signal, not the capability one."),
  H2("6.4  Capability anchoring (RQ3)"),
  PJ("Every pair’s capability point estimates are non-positive under NF4 at the reference budget, with one negligible exception (Mistral’s +0.9 percentage points on ARC, not significant). The loss is significant for Qwen3-1.7B on MMLU (−0.090, the study’s largest single effect), for Llama-3B on ARC (−0.032), and for Qwen3-4B on ARC (−0.016). The two capability benchmarks diverge informatively: Qwen3-1.7B’s large MMLU drop does not replicate on ARC (−0.009, not significant), so under that parser the dramatic within-Qwen scale gap looks partly MMLU-specific. That appearance is a parsing artefact rather than genuine MMLU-specificity. The option-letter parser falls back to a lenient \"first in-range capital letter anywhere\" scan whenever a response neither leads with the option letter nor states an explicit \"answer is X\". For qwen_2b_4bit that fallback fires on 48.7 percent of MMLU items and 52.3 percent of ARC items, against about 3 percent at fp16. It salvages the format-broken ARC answers more accurately (66 percent) than the MMLU ones (46 percent, well below the leading-letter rule’s 75 percent), and so masks the ARC loss. Part of the −0.090 MMLU gap therefore reflects a shift in answer format that the lenient parser scores less reliably, not only lost knowledge. Under a strict parser that scores an unparsable answer incorrect, ARC falls −0.343 and MMLU −0.293 — comparable large losses on both. The ARC drop above, scored by an identical protocol for both members, thus corroborates the direction of the loss, while its near-zero lenient magnitude reflects parser salvage rather than immunity. Each benchmark’s loss is honestly a bracket whose lenient end tracks answer-knowledge (largely preserved, since the salvaged answers are 66 percent correct) and whose strict end tracks format-compliant answering, which makes answer-format parsing a third measurement-dependence axis alongside the harm and over-refusal scorers. The capability anchor is what allows the safety deltas to be read correctly. The smallest Qwen pair is a capability story with a flat harm axis, and Llama’s harm decrease arrives together with its capability loss, which is more consistent with a capability side-effect than with a genuine alignment improvement. That is why it is labelled capability collapse masquerading as safety. The label encodes the correlation between the two axes; it does not by itself establish that the capability loss caused the safety change."),
  H2("6.5  Mechanism: refusal-margin geometry"),
  PJ("To probe the boundary behaviour behind the 128-token signal, a derived analysis measured each model’s first-token refusal margin in fp16 and under NF4, following the observation that quantization most affects low-confidence, near-boundary decisions [21]. A thin baseline margin does predict which prompts flip, but only weakly within a family, and the flips are symmetric: quantization destabilises the decision boundary in both directions rather than eroding refusals one-directionally. This mechanism reading is budget-invariant, and at the reference budget its signature is visible directly in the outcome data: the Qwen3-1.7B pair’s thirty-two discordant prompts split exactly sixteen to sixteen. The change is more consistent with generic capability-driven boundary instability than a targeted alignment shift. This reinforces the capability-anchored reading."),
  H2("6.6  Precision point: fp16 → INT8 → NF4 (RQ5)"),
  PJ("Adding INT8 shows the effect is not a smooth function of bit-width. On capability the result is a clean cliff at four-bit: every INT8 MMLU and ARC point estimate sits within about 1.3 percentage points of fp16 — paired per-item McNemar contrasts, none significant — far below the significant NF4 losses of three to nine points, while the significant capability losses are all at NF4. On safety, at the reference budget there is no robust move at either precision step. The one apparent counter-example from the 128-token run, a Llama-3B increase specific to INT8 (+0.040, then significant under both judges), does not replicate at 512 tokens under either judge (classifier Δ+0.005, McNemar p = 1.000; GPT-4o Δ+0.010, p = 0.688): it was a 128-era budget artefact, attributable, since Llama truncates at only 3 to 4 percent (§6.3), to cross-run greedy divergence on thin-margin prompts rather than to truncation, not a stable property of LLM.int8 on this model. Table 6.3 shows the sweep."),
  ...
  PJ("With 200 HarmBench prompts the confidence intervals are wide (about ±0.05), and a power analysis [18] makes the detection floor explicit: at n = 200 and the observed discordant rates, the minimum detectable ΔASR for 80 percent power is about 0.06 at the representative (median-discordant) pair, with the five pairs spanning 0.044 to 0.086. The predominance of nulls on the safety axis is therefore a power-bounded null (\"no detectable change at n = 200\"), not a proven zero, and the multi-seed sign-consistency for the smallest pair (§6.3) is compatible with a real effect below that floor."),
  PJ("Applying a Benjamini-Hochberg false-discovery-rate correction (q < 0.05) over the family of twenty primary NF4-versus-fp16 contrasts (exact McNemar p per contrast; results_512/analysis/multiple_comparisons.json), exactly three survive, and none is a harmful-compliance change: Qwen3-1.7B MMLU (−0.090, q = 0.008), Llama-3B ARC (−0.032, q = 0.008), and Phi-4-mini over-refusal (−0.048, q = 0.012, a decrease). The two capability survivors rest on deterministic exact-match scoring (MMLU and ARC parse an option letter; no refusal scorer is involved); the over-refusal survivor is itself regex-scored, so it inherits the same weaker-construct status as the demoted proxy. An independent three-class refusal judge does not reproduce it, and the completed human refusal audit (§7.2) finds that judge far better aligned with the annotator than the regex, so the survivor is most plausibly a measurement artifact of the regex scorer, not a classifier- or human-validated over-refusal shift. Not one HarmBench ASR contrast survives, including Llama’s individually significant decrease. The multiplicity-robust signal of four-bit NF4 is therefore capability degradation, plus one benign-direction over-refusal change that does not survive an independent scorer, and never a safety regression. This is the thesis’s headline empirical claim."),
  PJ("Every over-refusal member of that family is regex-scored, and the §7.2 human audit later found that scorer poorly aligned with a blinded human annotator on exactly this construct. A validation-informed parallel correction is therefore reported alongside it, clearly labelled post-hoc (it was added after both scorers’ results were known), with its composition fixed in a dated pre-analysis note before it was computed (docs/VALIDATION_INFORMED_FAMILY_NOTE.md). The parallel family recomputes the identical twenty contrasts with one change only: the five over-refusal contrasts are scored by the independent three-class judge under the strict mapping, which is the replication definition the judge-validation protocol pre-registered. The other fifteen p-values are reused verbatim and the same Benjamini-Hochberg procedure runs at the same threshold. " + CLAIMS.render.dual_family_sentence.replace(/\s—\s/g, ", ") + " The parallel family is purely deflationary, since no contrast becomes significant that was not already, and the regex-scored family above remains the pre-specified family of record for traceability. The substantive RQ2 conclusion is carried in the scorer-invariant form: no scorer finds a statistically significant over-refusal increase in any pair. Point estimates do move upward in places (the judge-strict Qwen3-1.7B +0.040 is the largest, McNemar p = 0.087) but none reaches significance under either scorer. Per-contrast values are in results_512/analysis/multiple_comparisons_judge_strict.json."),
];

const ch7 = [
  H1("Chapter 7  Discussion and Threats to Validity"),
  PJ("The results answer the research questions with calibrated claims. At the reference budget NF4 does not significantly increase harmful compliance under direct requests in any pair (RQ1); over-refusal does not rise, no scorer finds a statistically significant over-refusal increase in any pair, and its only exact-test-significant change is a regex-scored decrease that the human-grounded judge marks as most plausibly a measurement artifact (RQ2); capability loss is real, modest, partly benchmark-specific, and the only class of effect that robustly survives multiplicity correction under either scoring of the family, the one over-refusal survivor being most plausibly a regex measurement artifact (RQ3); the smallest model is the most capability-sensitive but shows no genuine safety shift once truncation is controlled (RQ4); and the cross-family, cross-precision picture is a consistent power-bounded null on safety with a capability cliff at four-bit (RQ5). This sits squarely with the one prior study that, like this one, isolates quantization at four bits: Hong et al. [8], working across a broader trust taxonomy and larger models, likewise find that four-bit compression broadly retains trustworthiness, and the matched-pair, capability-anchored design here extends that picture to compact models with a classifier-validated harmful-compliance scorer. For deployment the practical message is capability-shaped: eight-bit quantization is essentially free, four-bit costs capability (worst in the smallest model), and neither produced a robust safety cost under direct requests. Teams should evaluate the exact quantized artefact they will ship, at the deployment generation budget rather than a truncated one, and prefer the largest model that fits the memory budget over aggressively quantizing a smaller one. These recommendations are established for on-the-fly bitsandbytes quantization under direct harmful requests; other deployment formats (GGUF, GPTQ, AWQ) and attack-mediated threat models may behave differently and are future work."),
  ...
  PJ("The primary safety construct is scored by the benchmark’s own fine-tuned classifier rather than refusal matching, and the cross-judge agreement (Cohen’s κ 0.68 to 0.95 across all ten models at the reference budget) shows the finding is not an artefact of one classifier. A stratified human-label audit (200 items, single annotator, HarmBench rubric) now grounds this against truth: the classifier agrees with the human at Cohen’s κ 0.59 versus the regex’s 0.11, so the primary scorer is closer to human judgment than the demoted regex, not merely to another model. Two sampling caveats scope those figures symmetrically: the subset deliberately over-weights judge-vs-regex disagreement cases (roughly three times their population rate), so the κ values are a disagreement-enriched contrast rather than a population estimate; and the annotation view was capped at the first 2,000 characters of each response while the scorers saw the full text, which plausibly inflates measured disagreement, though the direction is not strictly identified, since a harmful-looking prefix followed by a benign completion cuts the other way. The residual is honestly bounded, κ 0.59 is moderate, not near-perfect, and rests on one annotator, so a second annotator with an inter-rater κ is the remaining strengthening. A third, open-weight judge (Llama-Guard-3-8B [22], revision-pinned) re-scored the 200 HarmBench generations of all fifteen aliases in the three-precision study with zero parse errors. Over the ten base/NF4 aliases of the primary study it agrees with the primary classifier at Cohen’s κ 0.36 to 0.92 per alias (highest for Llama, 0.89/0.92; lowest for Phi, 0.36/0.38); every per-pair Llama-Guard ΔASR is non-significant (the largest, Qwen3-1.7B’s −0.050 at McNemar p = 0.076, is a decrease); and the two scorers’ per-pair deltas agree in sign wherever the classifier delta is nonzero. This retires the reproducibility caveat that the cross-check rested solely on a versioned proprietary API snapshot. One construct boundary is stated plainly: Llama-Guard answers a looser question (\"is this response unsafe?\") than the classifier (\"is this generation an instance of the specific harmful behaviour?\"), so where it disagrees, as in the low Phi κ, the divergence is expected and non-diagnostic; it is a reproducibility cross-check, not a second construct-validity oracle. Over-refusal remains regex-scored, a known weaker construct, and the single over-refusal contrast that survives multiplicity correction (Phi-4-mini’s −0.048 decrease) inherits that same weaker-construct status. To test that survivor for scorer-robustness (the same question the classifier settled on the harmful-compliance axis), the 3,750 saved benign XSTest responses were re-scored by an independent three-class refusal judge (GPT-4o, pinned dated snapshot, temperature 0, zero parse failures) applying the XSTest paper’s own full-compliance / partial-refusal / full-refusal taxonomy [9], run as a sensitivity layer that leaves the regex primary and the multiplicity family unchanged. The judge does not reproduce the −0.048 decrease: Phi’s judge ΔOR is +0.016 (strict, direction reversed; McNemar p = 0.597) or −0.004 (broad; p = 1.000), neither significant, and the two scorers agree only poorly to moderately on benign over-refusal (Cohen κ −0.01 to 0.50 across the ten base/NF4 aliases of the primary study, with the judge counting roughly four times as many refusals). The one multiplicity-robust over-refusal contrast is therefore scorer-dependent, and a completed pre-registered human audit now settles which scorer deserves the weight. All 200 items of a gold set drawn from the same saved responses, deliberately over-sampling the regex-versus-judge disagreements (120 of 200) and presented shuffled and label-blind, were labelled by a single human annotator (the author) under the same three-class XSTest taxonomy the judge applies. The read-off, locked in a pre-registered outcome matrix before any label existed, gives a mechanical Outcome J: the judge aligns substantially better with the annotator than the regex (strict-mapping Cohen’s κ 0.485 against −0.006, broad-mapping 0.662 against 0.054). One protocol deviation is disclosed rather than glossed (pre-registration §8): the blindness exclusion was applied prompt-wide (the five quoted prompts were removed from all ten aliases) instead of to the at most ten pre-specified (alias, prompt) items, which under the pre-registration’s strict letter voids the clean confirmatory badge, so this is reported as a validation consistent with Outcome J that carries one disclosed deviation, not a clean pre-registered confirmation. The deviation was fixed in code before the draw and before any label existed, is blindness-conservative (it removes more potential priming, never less), and its footprint is bounded: a no-exclusion counterfactual draw overlaps the labelled set on 186 of 200 items (so at most 14 were displaced), and on those shared items the strict κ gap is +0.51, as large as the full-sample +0.49, so the judge-grounded reading does not depend on the deviation-specific items. The two failure modes differ in kind. The regex detected only 2 of the annotator’s 63 full refusals; the missed refusals are typically long, polite, lecture-style declinations that carry none of the template phrases. The judge caught 61 of 63 and errs instead by over-flagging, with its over-flags concentrated on the genuinely subjective partial-versus-full boundary (three-class exact agreement 0.695). Three bounds are stated plainly: these labels are a blinded single-annotator reference set, not ground truth; the draw is disagreement-enriched, so the κ values are a contested-slice contrast and no population over-refusal rate or ΔOR is estimated from them; and the shared benchmark taxonomy structurally favours a semantic judge over a phrase-matcher, while the pre-registered no-swap rule was locked before the judge results existed but after the regex results were known, a timing disclosed rather than argued from. The regex accordingly remains the original pre-specified scorer-of-record and the registered multiplicity family is unchanged, but the Phi-4-mini −0.048 contrast is carried as most plausibly a measurement artifact of the regex scorer, and the §6.7 validation-informed parallel family gives that reading a multiplicity-controlled footing (two survivors, both capability effects). The committed aggregate is results_512/analysis/xstest_human_validation.json (counts and rates only), with a redacted per-item audit trail in results_512/analysis/xstest_human_validation_items.jsonl (IDs and labels only); the annotation sheet holds raw text and stays local, and a second annotator with an inter-rater κ is the residual strengthening, exactly as for the harmful-compliance audit. This extends the thesis’s central scorer-sensitivity message from the harmful-compliance axis to the over-refusal axis. A second construct boundary is the threat model: the safety axis measures harmful compliance under direct requests only, with no attack augmentation, so the null does not speak to adversarial settings, whether optimised jailbreak attacks [5] or quantization-targeted poisoning that surfaces only after quantization [7], which is precisely where prior quantization-safety work reports effects."),
  H2("7.3  External and statistical-conclusion validity"),
  PJ("External validity is bounded by five pairs, one decoding regime (greedy primary, with a five-pair multi-seed arm; the two Qwen3 pairs run in non-default non-thinking mode, with the quantization × thinking-mode interaction untested), two bitsandbytes methods, direct requests only, and English-only text; the cross-family, cross-precision, and cross-budget sweeps widen it but do not make it general. Statistical-conclusion validity is bounded by sample size and is addressed head-on: paired-bootstrap intervals, exact McNemar tests, an explicit BH-FDR correction over the primary family, a minimum-detectable-effect analysis (about 0.06 at the representative pair; 0.044 to 0.086 across the five), and a two-layer evidence status rather than bare significance."),
];
...
const ch8 = [
  ...
  BUL("Sample size and power: 200 HarmBench prompts give wide (about ±0.05) confidence intervals; the minimum detectable ΔASR at 80% power is about 0.06 at the representative pair (0.044 to 0.086 across the five), so effects below that floor cannot be ruled out (the smallest pair’s multi-seed sign-consistency is compatible with one)."),
  BUL("Decoding: the primary results condition on a single greedy decode; the multi-seed sensitivity arm now covers all five pairs (five seeds each at temperature 0.7) and corroborates the null, with one disclosed wrinkle: for Mistral and Phi the greedy estimate falls marginally outside the seed range with a sign flip (small noise around zero), so the greedy point is not everywhere inside the stochastic range."),
  BUL("Judge grounding: the HarmBench classifier is now grounded against human labels on a stratified 200-item subset (classifier κ 0.59 vs regex 0.11, single annotator), alongside the second-judge cross-check, and the XSTest refusal judge is now grounded the same way by a completed 200-item blinded audit (judge strict κ 0.485 vs regex −0.006, a mechanical Outcome J carried with one disclosed protocol deviation, prereg §8); both groundings rest on one annotator, so a second annotator for an inter-rater κ remains the strengthening on both axes; an open-weight third judge (Llama-Guard-3-8B) additionally makes the HarmBench cross-check revision-pinned and locally rerunnable, though it scores a looser construct (§7.2)."),
...
const ch9 = [
  ...
  NUM("An adversarial-attack arm: re-run the HarmBench axis under at least one optimisation-based or LLM-mediated attack (for example GCG or PAIR) on a subset of pairs, upgrading the direct-request compliance result to a quantization-versus-safety-under-attack result. This is the most important extension of the safety axis.", "fw"),
  NUM("A second human annotator (inter-rater κ and adjudication of disagreements) extending the completed single-annotator human validations on both axes: HarmBench harmful compliance (classifier κ 0.59 vs regex 0.11 against human labels) and the XSTest refusal gold set, which is now completed as well (200 items, blinded, a mechanical Outcome J carried with one disclosed protocol deviation, prereg §8, with the judge substantially better aligned with the annotator than the regex). The open-weight guard-model cross-check (Llama Guard [22]) has since been run as a third judge (§7.2), so the residual on that axis is the annotator, not the instrument.", "fw"),
...
const refs = [
  ...
  REF(4, "A. Kharinaev et al., \"Investigating the impact of quantization methods on the safety and reliability of large language models,\" IEEE Access, vol. 14, pp. 96771 to 96793, 2026, doi: 10.1109/ACCESS.2026.3703899. arXiv:2502.15799."),
  ...
  REF(8, "J. Hong, J. Duan, C. Zhang, Z. Li, C. Xie, K. Lieberman, J. Diffenderfer, B. R. Bartoldson, A. K. Jaiswal, K. Xu, B. Kailkhura, D. Hendrycks, D. Song, Z. Wang, and B. Li, \"Decoding compressed trust: Scrutinizing the trustworthiness of efficient LLMs under compression,\" in Proc. International Conference on Machine Learning (ICML), PMLR vol. 235, 2024. arXiv:2403.15447."),
  REF(9, "P. Röttger, H. R. Kirk, B. Vidgen, G. Attanasio, F. Bianchi, and D. Hovy, \"XSTest: A test suite for identifying exaggerated safety behaviours in large language models,\" in Proc. NAACL, 2024. arXiv:2308.01263."),
  REF(10, "P. Liang et al., \"Holistic evaluation of language models,\" Transactions on Machine Learning Research (TMLR), 2023. arXiv:2211.09110."),
  REF(11, "J. Gu et al., \"A survey on LLM-as-a-judge,\" arXiv preprint arXiv:2411.15594, 2024."),
  REF(12, "M. Krumdick et al., \"No free labels: Limitations of LLM-as-a-judge without human grounding,\" arXiv preprint arXiv:2503.05061, 2025."),
  REF(13, "D. Hendrycks, C. Burns, S. Basart, A. Zou, M. Mazeika, D. Song, and J. Steinhardt, \"Measuring massive multitask language understanding,\" in Proc. International Conference on Learning Representations (ICLR), 2021. arXiv:2009.03300."),
  REF(14, "P. Clark, I. Cowhey, O. Etzioni, T. Khot, A. Sabharwal, C. Schoenick, and O. Tafjord, \"Think you have solved question answering? Try ARC, the AI2 reasoning challenge,\" arXiv preprint arXiv:1803.05457, 2018."),
  REF(15, "B. Efron and R. J. Tibshirani, An Introduction to the Bootstrap. New York: Chapman & Hall, 1993."),
  REF(16, "Q. McNemar, \"Note on the sampling error of the difference between correlated proportions or percentages,\" Psychometrika, vol. 12, no. 2, pp. 153 to 157, 1947."),
  REF(17, "Y. Benjamini and Y. Hochberg, \"Controlling the false discovery rate: A practical and powerful approach to multiple testing,\" Journal of the Royal Statistical Society: Series B (Methodological), vol. 57, no. 1, pp. 289-300, 1995."),
  REF(18, "R. J. Connor, \"Sample size for testing differences in proportions for the paired-sample design,\" Biometrics, vol. 43, no. 1, pp. 207-211, 1987."),
  REF(19, "J. Cohen, \"A coefficient of agreement for nominal scales,\" Educational and Psychological Measurement, vol. 20, no. 1, pp. 37-46, 1960."),
  REF(20, "J. R. Landis and G. G. Koch, \"The measurement of observer agreement for categorical data,\" Biometrics, vol. 33, no. 1, pp. 159-174, 1977."),
  REF(21, "I. Proskurina, L. Brun, G. Metzler, and J. Velcin, \"When quantization affects confidence of large language models?,\" in Findings of NAACL, 2024. arXiv:2405.00632."),
  REF(22, "H. Inan et al., \"Llama Guard: LLM-based input-output safeguard for human-AI conversations,\" Meta AI, 2023. arXiv:2312.06674."),
  REF(23, "J. Pineau et al., \"Improving reproducibility in machine learning research (a report from the NeurIPS 2019 reproducibility program),\" Journal of Machine Learning Research (JMLR), vol. 22, 2021. arXiv:2003.12206."),
  REF(24, "G. K. Sandve, A. Nekrutenko, J. Taylor, and E. Hovig, \"Ten simple rules for reproducible computational research,\" PLOS Computational Biology, vol. 9, no. 10, e1003285, 2013."),
  REF(25, "G. Wilson, J. Bryan, K. Cranston, J. Kitzes, L. Nederbragt, and T. K. Teal, \"Good enough practices in scientific computing,\" PLOS Computational Biology, vol. 13, no. 6, e1005510, 2017."),
  REF(26, "A. M. Smith et al., \"Journal of Open Source Software (JOSS): Design and first-year review,\" arXiv preprint arXiv:1707.02264, 2017."),
];

const appendix = [
  H1("Appendix A  Reproducibility Statement"),
  PJ("All code, configuration, and redacted result artefacts are in the project repository. The analysis reproduces byte-for-byte from the committed sidecars without a GPU; full experiments require the gated model weights and a CUDA GPU. The project maps to the ML Reproducibility Checklist [23]: models and algorithms are described (Chapters 3 to 4); datasets, splits, and sample counts are recorded in each run summary; code and declared dependencies are released; hyper-parameters and decoding controls are recorded in every summary; the compute infrastructure is recorded; and significance procedures are specified, with multiple comparisons disclosed. The dependency set is declared with minimum-version floors (a lockfile with exact pins is a future hardening step). The repository follows recognised research-software practice (scripts rather than manual steps, fixed seeds, explicit dependencies, and a documented project structure [24], [25]) and is packaged for reuse in line with open-source-software peer-review norms [26]. Raw generations are gitignored and hash-pinned; only redacted sidecars (identifiers, booleans, scalars) are released, so the study is reproducible without redistributing harmful text. The primary artefacts live under results_512/ (the 512-token study), with the 128-token run retained under results/ as the generation-length comparison; a checksum manifest pins 340 immutable raw files across both trees and the multi-seed arm."),
  H1("Appendix B  Artefacts and Test Suite"),
  PJ("The framework ships an automated verification suite across the schema, loaders (including the fail-loud quantization guard), scorers, analysis mathematics, the SLURM generator, and the registered document surfaces. The artifact-derived claim registry and claim checker bind load-bearing tables, captions, deck data, and recurring prose to results_512/analysis/, while a checksum manifest pins immutable raw artefacts. The live test inventory is obtained from pytest rather than copied into the thesis."),
];
