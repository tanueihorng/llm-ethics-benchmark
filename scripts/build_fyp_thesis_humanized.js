...
const ch8 = [
  ...
  BUL("Judge grounding: the HarmBench classifier is now grounded against human labels on a stratified 200-item subset (classifier κ 0.59 vs regex 0.11, single annotator), alongside the second-judge cross-check, and the XSTest refusal judge is now grounded the same way by a completed 200-item blinded audit (judge strict κ 0.485 vs regex −0.006, pre-registered Outcome J); both groundings rest on one annotator, so a second annotator for an inter-rater κ remains the strengthening on both axes."),
