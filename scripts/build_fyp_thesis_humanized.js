...
const ch6 = [
  ...
  PJ("Over-refusal moves in the benign direction where it moves at all: the only over-refusal delta significant under the exact paired test is Phi-4-mini’s decrease (ΔOR = −0.048, CI [−0.076, −0.020]) under the primary regex scorer, and an independent refusal judge does not reproduce it (§7.2), so that survivor is scorer-dependent. Qwen3-1.7B shows a non-significant decrease (−0.024, McNemar p = 0.109). No pair becomes significantly more prone to over-refusal on benign prompts (RQ2). The Qwen3-1.7B pair, whose +0.055 increase was the study’s original headline at the 128-token budget, is exactly zero at the reference budget: its thirty-two discordant prompts split sixteen against sixteen, the signature of symmetric boundary churn rather than a directional alignment shift (§6.5)."),
