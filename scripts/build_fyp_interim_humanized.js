...
// `make report` / `make thesis` never touch it, and it never touches them.
// Output: docs/FYP_Interim_2026-07-10_humanized.docx   (build: make interim)
// Purpose: a fuller-but-shorter INTERIM milestone report (~25-30 pp) that keeps
// the headline results and adds interim-appropriate framing (progress to date,
// challenges/corrections, remaining work). The full report (build_fyp_report_v5)
// stays as the final-thesis-grade master.
// ============================================================================
const fs = require("fs");
...
const cover = [
  ...
  CTR("FINAL YEAR PROJECT", { size: 24, bold: true, after: 60 }),
  CTR("Interim Report", { size: 28, bold: true, after: 480 }),
  ...
  CTR("Interim Report · 10 July 2026 · content revised through 22 July 2026", { size: 26, bold: true, after: 80 }),
...
const ch1 = [
  ...
  PJ("This interim report is organised around five research questions:"),
...
const ch3 = [
  ...
  PJ("Table 3.1 lists the five pairs. They span four families and roughly 1.7 to 7.2 billion parameters, which gives both within-family scale contrasts (the two Qwen models) and cross-family generality."),
  ...
  PJ("HarmBench Attack Success Rate is scored by the official HarmBench classifier (cais/HarmBench-Llama-2-13b-cls) [3] — prompted with the Hugging Face model-card template rather than the GitHub evaluation-harness variant, so absolute ASR levels are not directly harness-comparable while matched-pair deltas are unaffected — run in full precision so that the reference instrument is not itself confounded by quantization. A second, architecturally independent judge (GPT-4o; run 2026-06-14 — the API alias's dated snapshot was not recorded, so the revision-pinned open-weight LlamaGuard third judge is the reproducible arm) re-scores the same generations as a robustness check [11], [12]. A deterministic refusal regex is retained only as a secondary, transparent proxy. This choice is itself a research object: this report measures the agreement between scorers and shows that the regex over-counts, which is why it is demoted. MMLU and ARC use deterministic exact match; XSTest over-refusal is scored by the regex, since the official HarmBench classifier judges harmful compliance only. XSTest's own native evaluation relies on LLM-based three-way labelling (full compliance, partial refusal, full refusal) rather than a released fine-tuned classifier; retaining a deterministic binary refusal regex here is a deliberate, reproducibility-motivated deviation that yields a coarser refusal-versus-non-refusal construct, acknowledged as a weaker-construct limitation in Chapter 8."),
...
const ch6 = [
  ...
  PJ("With 200 HarmBench prompts the confidence intervals are wide (about ±0.05), and a power analysis [18] makes the detection floor explicit: at n = 200 and the observed discordant rates, the minimum detectable ΔASR for 80 percent power is about 0.06 for a representative pair (the per-pair values span 0.04 to 0.09). The predominance of nulls on the safety axis is therefore a power-bounded null (\"no detectable change at n = 200\"), not a proven zero, and the multi-seed sign-consistency for the smallest pair (§6.3) is compatible with a real effect below that floor."),
  PJ("Applying a Benjamini-Hochberg false-discovery-rate correction (q < 0.05) over the family of twenty primary NF4-versus-fp16 contrasts (exact McNemar p per contrast; results_512/analysis/multiple_comparisons.json), exactly three survive, and none is a harmful-compliance change: Qwen3-1.7B MMLU (−0.090, q = 0.008), Llama-3B ARC (−0.032, q = 0.008), and Phi-4-mini over-refusal (−0.048, q = 0.012, a decrease). Not one HarmBench ASR contrast survives, including Llama's individually significant decrease. The multiplicity-robust signal of four-bit NF4 is therefore capability degradation, plus one benign-direction over-refusal change, and never a safety regression. That one over-refusal survivor, Phi-4-mini's −0.048, is regex-scored, so it inherits the same weaker-construct status as the demoted proxy and should be read as a reproducible non-refusal-rate decrease rather than a classifier-validated over-refusal shift. This is the study's headline empirical claim. A validation-informed parallel correction, the identical twenty-contrast family with the five over-refusal contrasts scored by the independent judge under its strict mapping and its composition locked in a dated note before computation (docs/VALIDATION_INFORMED_FAMILY_NOTE.md; results_512/analysis/multiple_comparisons_judge_strict.json), leaves two contrasts surviving, both capability effects (Qwen3-1.7B MMLU, q = 0.008; Llama-3B ARC, q = 0.008); it is purely deflationary, and the registered regex-scored family above remains the family of record."),
  PJ("A refusal-judge sensitivity check bears directly on that one survivor. An independent three-way refusal judge (GPT-4o, XSTest's own full-compliance / partial-refusal / full-refusal labelling) re-scored all 3,750 saved benign responses (no parse failures), with the regex kept as the primary scorer and the twenty-contrast BH-FDR family unchanged, and it does not reproduce the −0.048 decrease: scoring full refusals only, Phi-4-mini's judge ΔOR is +0.016 (CI [−0.028, +0.060], McNemar p = 0.597), the opposite direction, and counting partial or full refusals it is −0.004 (CI [−0.048, +0.036], p = 1.000). The two scorers disagree substantially, the judge flags about four times more refusals (mean over-refusal 0.171 against the regex's 0.044) and per-alias Cohen's κ runs from −0.01 to 0.50, and no pair shows a significant judge ΔOR, which agrees with the RQ2 null but with a direction and magnitude that differ from the regex, most sharply for this survivor. The one over-refusal survivor is therefore scorer-dependent, and a completed 200-item blinded human audit (single annotator, disagreement-enriched draw, a mechanical Outcome J carried with one disclosed protocol deviation, pre-registration §8) now resolves the weighting: the judge aligns substantially better with the annotator than the regex on this construct (strict κ 0.485 versus −0.006; the regex recovered only 2 of the annotator's 63 full refusals), so the contrast is carried as most plausibly a measurement artifact of the regex scorer, with the regex retained as the original pre-specified scorer-of-record and a second annotator the residual strengthening (results_512/analysis/xstest_judge_agreement.json; results_512/analysis/xstest_human_validation.json; results_512/analysis/xstest_human_validation_items.jsonl, IDs and labels only)."),
];

const ch7 = [
  H1("Chapter 7  Discussion and Threats to Validity"),
  PJ("The results answer the research questions with calibrated, not sweeping, claims. At the reference budget NF4 does not significantly increase harmful compliance under direct requests in any pair (RQ1); no scorer finds a statistically significant over-refusal increase in any pair, and the only exact-test-significant over-refusal change is a regex-scored decrease (RQ2); capability loss is real, modest, partly benchmark-specific, and the only class of effect that robustly survives multiplicity correction, the one over-refusal survivor being scorer-dependent (RQ3); the smallest model is the most capability-sensitive but shows no genuine safety shift once truncation is controlled (RQ4); and the cross-family, cross-precision picture is a consistent power-bounded null on safety with a capability cliff at four-bit (RQ5). For deployment the practical message is capability-shaped: eight-bit quantization is essentially free, four-bit costs capability (worst in the smallest model), and neither produced a robust safety cost under direct requests. Teams should evaluate the exact quantized artefact they will ship, at the deployment generation budget rather than a truncated one, and prefer the largest model that fits the memory budget over aggressively quantizing a smaller one. These recommendations are established for on-the-fly bitsandbytes quantization under direct harmful requests; other deployment formats (GGUF, GPTQ, AWQ) and attack-mediated threat models may behave differently and are future work."),
...
const ch8 = [
  ...
  BUL("Sample size and power: 200 HarmBench prompts give wide (about ±0.05) confidence intervals; the minimum detectable ΔASR at 80% power is about 0.06 for a representative pair (0.04 to 0.09 across pairs), so effects below that floor cannot be ruled out (the smallest pair's multi-seed sign-consistency is compatible with one)."),
  BUL("Decoding: the primary results condition on a single greedy decode; the multi-seed sensitivity arm now covers all five pairs and corroborates the null, though for Mistral and Phi the greedy estimate falls marginally outside the seed range with a sign flip (small noise around zero)."),
  BUL("Judge grounding: the HarmBench classifier is now grounded against human labels on a stratified 200-item subset (classifier κ 0.59 vs regex 0.11, single annotator), alongside the second-judge cross-check, and the over-refusal axis now carries its own 200-item blinded audit (judge strict κ 0.485 vs regex −0.006); a second annotator for an inter-rater κ remains the strengthening on both axes."),
...
const ch9 = [
  ...
  NUM("A second human annotator (inter-rater κ and adjudication of disagreements) extending the completed single-annotator human validation (classifier κ 0.59 vs regex 0.11 against human labels).", "fw"),
...
const out = "/Users/tanueihorng/fyp_quant/docs/FYP_Interim_2026-07-10_humanized.docx";
