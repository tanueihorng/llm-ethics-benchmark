...
const appendix = [
  ...
  PJ("All code, configuration, and redacted result artefacts are in the project repository. The analysis reproduces byte-for-byte from the committed sidecars without a GPU; full experiments require the gated model weights and a CUDA GPU. The project maps to the ML Reproducibility Checklist [19]: models and algorithms are described (Chapters 3 to 4); datasets, splits, and sample counts are recorded in each run summary; code and pinned dependencies are released; hyper-parameters and decoding controls are recorded in every summary; the compute infrastructure is recorded; and significance procedures are specified, with multiple comparisons disclosed. The repository follows recognised research-software practice (scripts rather than manual steps, fixed seeds, explicit dependencies, and a documented project structure [20], [21]) and is packaged for reuse in line with open-source-software peer-review norms [22]. Raw generations are gitignored and hash-pinned; only redacted sidecars (identifiers, booleans, scalars) are released, so the study is reproducible without redistributing harmful text. The primary artefacts live under results_512/ (the 512-token study), with the 128-token run retained under results/ as the generation-length comparison; a checksum manifest pins 300 immutable raw files across both trees and the multi-seed arm."),
