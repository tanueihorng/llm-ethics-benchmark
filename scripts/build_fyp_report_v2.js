...
//   FYP_BUILD_SUPERSEDED=1 node scripts/build_fyp_report_v2.js
// ============================================================================
if (!process.env.FYP_BUILD_SUPERSEDED) {
  console.error(
    "SUPERSEDED builder (scripts/build_fyp_report_v2.js): refusing to build - its output is a " +
...
// FYP Report builder (v2 — publication-grade pass) — Word document via docx-js
// Output: /Users/tanueihorng/fyp_quant/docs/FYP_Report_2026-06-26_v2.docx
// NOTE: this is the publication-grade revision (figures, IEEE citations, FDR/power,
// scorer-validation lead). The original interim report (build_fyp_report.js →
// FYP_Report_2026-06-14.docx) is left untouched for side-by-side comparison.
...
const ch1 = [
  ...
  ...numberedList([
    ...
    "An open, reproducible benchmarking framework (approximately 2,800 lines of production Python and 1,800 lines of automated tests; 306 tests across twenty-three files) with batched chat-templated generation, per-prompt audit logging, immutable hash-pinned raw artefacts, redacted score sidecars, and resumable per-model SLURM matrix jobs for the NTU TC1 cluster.",
    "An empirical characterisation of quantization across five model pairs, four families, and three precisions (fp16 → INT8 → NF4): under the validated classifier, four-bit NF4 never significantly reduces harmful compliance and significantly increases it only in the smallest model, the multiplicity-robust signal is capability loss rather than safety change, and the effect is not bit-width-graded — supported by a refusal-margin mechanism probe, a multi-seed sensitivity arm, an FDR/power analysis, and a second-judge cross-check.",
  ]),
...
const OUTPUT = "/Users/tanueihorng/fyp_quant/docs/FYP_Report_2026-06-26_v2.docx";
