...
const ch8 = [
  ...
  BUL("Judge grounding: the HarmBench classifier is treated as reference without independent human-label validation; the second judge is a partial cross-check (an annotation sheet for a human gold set is prepared but not yet labelled)."),
  ...
  BUL("Generation budget: absolute ASR is budget-dependent by construction (complete generations give more room for harmful content to appear), which is controlled here by adopting HarmBench’s standardized 512-token budget; the 128-era signals (Qwen3-1.7B NF4 +0.055; Llama-3B INT8 +0.040) are truncation artefacts retained only as the generation-length comparison."),
...
const ch9 = [
  ...
  NUM("Human-grounded judge validation (a stratified human-labelled subset reporting classifier-vs-human and regex-vs-human agreement; the annotation sheet is prepared) and an open-weight guard model (e.g. Llama Guard [21]) for a fully reproducible cross-check.", "fw"),
...
const refs = [
  ...
  REF(5, "K. Egashira, M. Vero, R. Staab, J. He, and M. Vechev, “Exploiting LLM quantization,” in Proc. Advances in Neural Information Processing Systems (NeurIPS), 2024. arXiv:2405.18137."),
