...
const ch6 = [
  ...
  PJ("Over-refusal moves in the benign direction where it moves at all: the only over-refusal delta significant under the exact paired test is Phi-4-mini’s decrease (ΔOR = −0.048, CI [−0.076, −0.020], under the primary regex scorer; an independent refusal judge does not reproduce it, and a blinded human audit finds that judge substantially better aligned with the annotator than the regex, so the decrease is most plausibly a measurement artifact of the regex scorer — §7.2), while Qwen3-1.7B’s smaller decrease is not significant on either criterion (−0.024, McNemar p = 0.109; its CI [−0.048, 0.000] touches zero). No pair becomes significantly more prone to over-refusal on benign prompts, under either scorer (RQ2)."),
  PJ("The Qwen3-1.7B pair, whose +0.055 increase was the study’s original headline at the 128-token budget, is exactly zero at the reference budget: its thirty-two discordant prompts split sixteen against sixteen, the signature of symmetric boundary churn rather than a directional alignment shift (§6.5)."),
...
const ch8 = [
  ...
  BUL("Sample size and power: 200 HarmBench prompts give wide (about ±0.05) confidence intervals; the minimum detectable ΔASR at 80% power is about 0.06 at the representative pair (0.044–0.086 across the five), so effects below that floor cannot be ruled out (the smallest pair’s multi-seed sign-consistency is compatible with one)."),
  ...
  BUL("Judge grounding: the HarmBench classifier is now grounded against human labels on a stratified 200-item subset (classifier κ 0.59 vs regex 0.11, single annotator), alongside the second-judge cross-check, and the XSTest refusal judge is likewise grounded by a completed 200-item blinded audit (judge strict κ 0.485 vs regex −0.006, a mechanical Outcome J carried with one disclosed protocol deviation, prereg §8); both groundings rest on a single annotator, so a second annotator for an inter-rater κ remains the strengthening on both axes; an open-weight third judge (Llama-Guard-3-8B) additionally makes the HarmBench cross-check revision-pinned and locally rerunnable, though it scores a looser construct (§7.2)."),
...
const out = path.join(__dirname, "..", "docs", "FYP_Thesis_2026-07-02_v4.docx");
