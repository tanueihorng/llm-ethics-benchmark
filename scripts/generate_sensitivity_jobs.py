...
# --models lists exactly the aliases emitted by this generator invocation (the
# --pairs selection). The runner has NO skip logic, so it re-scores every alias
# listed; an incremental extension must regenerate with --pairs so this line
# names ONLY the new aliases and never re-scores (or requires the raw of) pairs
# already judged in an earlier run.
...
def main() -> int:
    ...
    # First emitted pair leads the run order (the load-bearing pair when the full
    # arm is generated; the first extension pair under `--pairs`). Derived from the
    # selection so a scoped regen never dangles a reference to an un-emitted alias.
    first_base, first_quant = selected[0][1][0], selected[0][1][1]
    # The judge scores exactly the aliases emitted here (respecting `--pairs`); an
    # incremental `--pairs` run therefore judges ONLY the new aliases and never
    # re-scores pairs generated in an earlier run.
    judge_note = (
        "(scores only the emitted pairs — "
        + ", ".join(pid for pid, _ in selected)
        + ")"
    )
    readme.write_text(
        ...
        "1. Generation, first pair first: "
        f"`sbatch {out_dir}/{first_base}__{args.suffix}.sbatch` then "
        f"`{first_quant}__{args.suffix}.sbatch` (repeat per pair).\n"
        "2. After generation for a seed completes, judge it: "
        f"`sbatch {out_dir}/judge_{args.suffix}_seed1.sbatch` {judge_note} (repeat per seed).\n"
