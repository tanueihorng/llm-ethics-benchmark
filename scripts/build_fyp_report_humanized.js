...
,
  ...
  PJ("Result 1: the v2 regex systematically over-counts ASR, unevenly by family. The judge ASR is far lower than the v2 non-refusal rate for the Qwen models (e.g. Qwen 1.7B base 0.255 vs 0.595; Qwen 4B base 0.115 vs 0.235) and close for Llama (0.100 vs 0.090). Agreement is correspondingly fair-to-moderate for Qwen and Mistral (κ ≈ 0.25 to 0.59) and high for Llama and Phi (κ ≈ 0.67 to 0.84). The reason is structural: the regex equates \"did not refuse\" with \"attack succeeded\", but many non-refusals are not genuinely harmful (vague deflections, safety lectures without refusal keywords, on-topic but benign answers, failed attempts). Qwen produces a large volume of such ambiguous middle-ground text; Llama mostly either refuses cleanly or complies, leaving little for the regex to mis-score. This is exactly the failure mode the HarmBench paper's fine-tuned classifier was built to avoid, and it is why the official classifier, not refusal counting, is adopted as the primary HarmBench scorer."),
...
const appendixH = [
  ...
  PJ("The classifier has since been cross-checked three times over: by an independent frontier-model second judge (gpt-4o, §6.12 Result 4), against human labels on a stratified subset (§6.12 Result 5), and by an open-weight guard model (Llama-Guard-3-8B, revision-pinned; §6.12), whose revision-pinned, locally rerunnable re-scoring, unlike the versioned API judge's, agrees with the classifier at κ 0.36–0.92 across the ten base/NF4 aliases and finds every per-pair ΔASR non-significant, with the construct caveat that it answers a looser safety question than the behavior-specific classifier. All three run through the pluggable judge backend in `ethical_benchmark/judges/validation.py` and the agreement analysis in `scripts/judge_agreement.py`; the primary scorer is unaffected either way."),
];
