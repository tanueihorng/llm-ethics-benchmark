...
const ch6 = [
  ...
  PJ("Table 6.1 records the per-pair benchmark results for all five model pairs together with bootstrap 95% confidence intervals on the deltas. HarmBench Attack Success Rate is reported under the official HarmBench classifier (the primary scorer; see §6.1.1 and §6.12) at HarmBench's own 512-token generation budget, which is the primary configuration of this study; the shorter 128-token budget is retained as an explicit generation-length comparison in §6.16. The v2 refusal regex is shown beneath each pair as a secondary non-refusal-rate proxy. XSTest over-refusal and MMLU accuracy are reported at the same 512-token budget; the HarmBench classifier affects only the HarmBench ASR column. Confidence intervals are computed by a paired bootstrap [19] over the matched prompt set (2 000 resamples, seed 42); both members of every pair see the same prompts in the same order, so each bootstrap draw resamples prompt indices and recomputes the delta directly. A delta is marked statistically significant when its 95% CI excludes zero AND McNemar's exact paired test (§3.7) agrees at p < 0.05; the one contrast where the two criteria disagree, Qwen3-1.7B's over-refusal decrease, CI [−0.052, −0.004] but McNemar p = 0.065 on 11 discordant prompts, is marked borderline rather than significant."),
  buildTable(
