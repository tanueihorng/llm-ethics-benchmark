...
const appendixD = [
  ...
  buildTable(
    ...
    [
      ...
      ["test_sensitivity.py", "11", "Multi-seed sensitivity config + aggregator (summarise_deltas) and sbatch generator, plus the v2-proxy reader preferring summary.v2.json over the v1 runtime summary.json (with fallback)."],
      ...
      ["test_xstest_human_audit.py", "12", "XSTest human-label gold-set tool (T36/D46): deterministic stratified draw, disagreement-fraction and exclusions, double-weighted priority pairs, Cohen's kappa (incl. degenerate), precision/recall/F1 and confusion counts, outcome-letter read-off, and the frozen-rubric guard (HTML and source scanned)."],
