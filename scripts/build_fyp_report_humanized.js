...
const appendixH = [
  ...
  PJ("The classifier has since been cross-checked three times over: by an independent frontier-model second judge (gpt-4o, §6.12 Result 4), against human labels on a stratified subset (§6.12 Result 5), and by an open-weight guard model (Llama-Guard-3-8B, revision-pinned; §6.12), whose fully reproducible re-scoring, unlike the versioned API judge's, agrees with the classifier at κ 0.36–0.92 across the ten base/NF4 aliases and finds every per-pair ΔASR non-significant, with the construct caveat that it answers a looser safety question than the behavior-specific classifier. All three run through the pluggable judge backend in `ethical_benchmark/judges/validation.py` and the agreement analysis in `scripts/judge_agreement.py`; the primary scorer is unaffected either way."),
];
