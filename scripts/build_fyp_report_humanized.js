...
const ch3 = [
  ...
  PJ("Three model pairs form the primary controlled design. The Qwen pairs at 1.7 billion and four billion parameters [16] are the primary within-family comparison and provide both a quantization axis and a scale axis. The Llama 3.2 3B pair [17] sits between them by parameter count and is a cross-family robustness check. Table 3.1 summarises this primary matrix; a cross-family extension later adds two further matched pairs (Mistral-7B and Phi-4-mini, introduced with the experimental setup in Chapter 5 and analysed in §6.13), taking the study to five pairs across four families."),
  buildTable(
...
,
  ...
  PJ("The Qwen3-1.7B and Qwen3-4B full-precision baselines reveal a capability-and-safety coupling within the Qwen family. At full precision (HarmBench under the official classifier at the 512-token reference budget), scaling from 1.7B to 4B parameters produces: HarmBench ASR 0.255 → 0.115 (−14.0 pp; the larger model produces fewer genuinely harmful completions), MMLU accuracy 0.643 → 0.747 (+10.4 pp; substantially more capable), and XSTest over-refusal 0.056 → 0.032 (−2.4 pp; fewer false refusals on benign prompts). Within Qwen the larger model dominates the smaller one on every dimension: more capable, more safety-calibrated, and better calibrated on benign prompts. (Under the v2 non-refusal proxy the baseline ASR gap looks larger because the proxy over-counts; the genuine harmful-compliance gap is the 14.0 pp classifier figure.) This baseline divergence motivates the matched-pair design: quantization effects must be interpreted against scale-dependent baselines."),
