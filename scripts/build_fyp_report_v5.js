...
const ch6 = [
  ...
  buildTable(
    ...
    [
      ...
      ["llama_3_2_3b", "HarmBench ASR (judge)", "0.100", "0.060", "−0.040 [−0.070, −0.010]", "yes"],
      ...
      ["mistral_7b", "HarmBench ASR (judge)", "0.585", "0.565", "−0.020 [−0.085, +0.040]", "no"],
  ...
  PJ("The headline finding for RQ5 is that, at the 512-token reference budget under the official classifier, NF4 quantization produces no significant harmful-compliance increase in any pair, and the only individually-significant ΔASR (Llama-3.2-3B, −0.040) is a decrease. The Qwen3-1.7B pair shows ΔASR at exactly zero paired with a large, significant capability loss (ΔMMLU −0.090); the Qwen3-4B pair shows a directional, non-significant +0.040 at preserved capability; the Llama-3.2-3B pair shows a small significant ASR decrease alongside a significant ARC loss (§6.4.1). The capability dimension is directionally consistent (all five pairs have negative ΔMMLU; the Qwen3-1.7B loss is the only MMLU-significant one, with further ARC-significant losses on Qwen3-4B and Llama-3.2-3B). On over-refusal, the only significant deltas are decreases. The composite picture is that NF4 quantization does not produce a uniform or significant safety regression across families and scales; the robust, multiplicity-surviving signals are all capability losses. See §6.11 for the full discussion and §6.13 for the cross-family extension, which confirms the pattern across Mistral-7B and Phi-4-mini (no significant ASR increase in either)."),
...
,
  ...
  PJ("Result 2: the choice of scorer changes the conclusion. Under the judge, at the 512-token reference budget, the per-pair HarmBench ΔASR values are: Qwen 1.7B 0.000 (CI [−0.055, +0.055], not significant); Qwen 4B +0.040 (CI [0.000, +0.080], not significant); Llama −0.040 (CI [−0.070, −0.010], individually significant and in the safety-improving direction). Compared with the v2 proxy (−0.025, +0.070, −0.020), the judge and the regex disagree in level and per-model ordering: the proxy's largest positive move is on Qwen 4B (+0.070) whereas the judge's only individually significant contrast is a decrease on Llama, and no HarmBench ΔASR contrast survives the Benjamini-Hochberg correction over the primary family. The interpretation labels follow: Qwen 1.7B is broad_degradation, but at 512 that label is driven entirely by its significant MMLU capability loss (ΔMMLU −0.090), with the harm axis flat (ΔASR 0.000) — it is a capability-only degradation, not a safety regression; Qwen 4B is alignment_degradation but only directionally (+0.040, not significant); Llama is capability_collapse_masquerading_as_safety (harmful compliance decreases and capability decreases together). The judge validation is therefore not a rubber stamp: it corrected a refusal-counting artefact, and at the reference budget it establishes that refusal-based scoring and the classifier lead to different per-model conclusions about where, and in which direction, harmful compliance moves. This is reported transparently as the central methodological result of the chapter, and the HarmBench numbers throughout the report are the judge values accordingly (D16)."),
