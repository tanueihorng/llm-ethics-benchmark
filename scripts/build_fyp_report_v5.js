...
const ch3 = [
  ...
  PJ("Three model pairs are evaluated. The Qwen pairs at 1.7 billion and four billion parameters [16] serve as the primary within-family comparison and provide both a quantization axis and a scale axis. The Llama 3.2 3B pair [17] sits between them by parameter count and serves as a cross-family robustness check. Table 3.1 summarises the model matrix."),
  buildTable(
...
const ch6 = [
  ...
  PJ("Table 6.1 records the per-pair benchmark results for all five model pairs together with bootstrap 95% confidence intervals on the deltas. HarmBench Attack Success Rate is reported under the official HarmBench classifier (the primary scorer; see §6.1.1 and §6.12) at HarmBench's own 512-token generation budget, which is the primary configuration of this study; the shorter 128-token budget is retained as an explicit generation-length comparison in §6.16. The v2 refusal regex is shown beneath each pair as a secondary non-refusal-rate proxy. XSTest over-refusal and MMLU accuracy are reported at the same 512-token budget; the HarmBench classifier affects only the HarmBench ASR column. Confidence intervals are computed by a paired bootstrap [18] over the matched prompt set (2 000 resamples, seed 42); both members of every pair see the same prompts in the same order, so each bootstrap draw resamples prompt indices and recomputes the delta directly. A delta is marked statistically significant when its 95% CI excludes zero."),
  buildTable(
    ...
    [
      ...
      ["qwen_2b", "XSTest over-refusal", "0.056", "0.028", "−0.028 [−0.052, −0.004]", "yes"],
  ...
  P("Table 6.2  All-pair quantization deltas, interpretation labels, and evidence status. Δ = 4-bit − baseline. HarmBench ΔASR (judge) is the primary scorer; the v2 proxy row is shown for transparency. ★ marks deltas whose paired-bootstrap 95% CI excludes zero. The McNemar row is the exact two-sided paired-binary test on the judge ΔASR (independent of the bootstrap). Under the judge at the 512-token reference budget, the deltas whose bootstrap CI excludes zero (★) are Llama-3.2-3B ΔASR (−0.040, a decrease; McNemar p = 0.021 — the only significant ΔASR, and no pair shows a significant increase), Qwen3-1.7B ΔMMLU (−0.090), and the two over-refusal decreases Qwen3-1.7B (−0.028) and Phi-4-mini (−0.044). The two-layer evidence status reports each label's statistical support separately from the label itself (§3.8): Qwen 4B keeps the alignment_degradation label by point estimate but is flagged directional because its ΔASR CI touches zero (McNemar p = 0.096); it is not a statistically confirmed regression. Labels are judge-primary (D16).", { size: 18 }),
  ...
  PJ("The headline finding for RQ5 is that, at the 512-token reference budget under the official classifier, NF4 quantization produces no significant harmful-compliance increase in any pair, and the only individually-significant ΔASR (Llama-3.2-3B, −0.040) is a decrease. The Qwen3-1.7B pair shows ΔASR at exactly zero paired with a large, significant capability loss (ΔMMLU −0.090); the Qwen3-4B pair shows a directional, non-significant +0.040 at preserved capability; the Llama-3.2-3B pair shows a small significant ASR decrease alongside a significant ARC loss (§6.4.1). The capability dimension is directionally consistent (all five pairs have negative ΔMMLU; the Qwen3-1.7B loss is the only MMLU-significant one, with further ARC-significant losses on Qwen3-4B and Llama-3.2-3B). On over-refusal, the only significant deltas are decreases. The composite picture is that NF4 quantization does not produce a significant or uniform safety regression in any family or at any scale; the robust, multiplicity-surviving signals are all capability losses. See §6.11 for the full discussion and §6.13 for the cross-family extension, which confirms the pattern across Mistral-7B and Phi-4-mini (no significant ASR increase in either)."),
...
,
  ...
  PJ("Yes, and this is the robust, budget-invariant cost of NF4. The Qwen3-1.7B model loses 9.0 percentage points of MMLU accuracy under NF4 quantization (0.643 → 0.553, significant), and this is the single largest, multiplicity-surviving effect in the study. Every pair's MMLU point estimate is negative (Qwen 4B −0.003, Llama −0.037, Mistral −0.020, Phi −0.027); on the second capability benchmark ARC-Challenge (§6.4.1) the losses that reach significance are Llama (−0.032) and Qwen 4B (−0.016), both of which also survive multiplicity correction. So capability degradation holds in direction across all five pairs, reaches significance on at least one benchmark for the Qwen 1.7B, Qwen 4B and Llama pairs, and is the only class of effect that survives the Benjamini–Hochberg correction. The answer to RQ3: NF4 quantization degrades general capability, most severely (and MMLU-significantly) in the 1.7B Qwen model, and this capability cost is robust to generation-length budget where the harmful-compliance change is not (§6.16). The one substantive cross-benchmark divergence is the Qwen 1.7B loss, which is far smaller and non-significant on ARC (−0.9 pp) than on MMLU (−9.0 pp), so its *severity* is partly MMLU-specific even though its direction is not."),
...
const appendixD = [
  ...
  buildTable(
    ...
    [
      ...
      ["test_report_claims.py", "1", "Claim lock (D43): runs scripts/verify_report_claims.py — every load-bearing number in the canonical report AND thesis builders is asserted against the committed analysis artifacts (53 checks); the documents cannot drift from their evidence without failing the suite."],
