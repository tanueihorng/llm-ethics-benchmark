...
// Output: /Users/tanueihorng/fyp_quant/docs/FYP_Report_2026-07-01_v5.docx
...
// Document constants
// ------------------------------------------------------------
const PAGE_W = 11906;       // A4 (NTU standard)
const PAGE_H = 16838;       // A4
const MARGIN = 1440;        // 1 inch (tables auto-fit CONTENT_W via the drift adjustment in the Table helper)
...
function PJ(text, opts = {}) {
  // Justified paragraph for body text (1.5 line spacing per NTU)
  return new Paragraph({
...
const ch1 = [
  ...
  ...numberedList([
    ...
    "An open, reproducible benchmarking framework (approximately 8,900 lines of production Python and 4,600 lines of automated tests; 382 tests across twenty-nine files) with batched chat-templated generation, per-prompt audit logging, immutable hash-pinned raw artefacts, redacted score sidecars, and resumable per-model SLURM matrix jobs for the NTU TC1 cluster.",
...
const ch3 = [
  ...
  PJ("Four-bit loading is forbidden on a CPU runtime: the loader raises a clear error when quantized is true and the resolved runtime device is not cuda. After load, the loader verifies that quantization actually engaged — checking is_loaded_in_4bit, is_loaded_in_8bit, is_quantized, and the presence of an hf_quantizer — and raises a RuntimeError refusing to proceed if none of those signals is set, so a silently unquantized load cannot be mislabelled as quantized."),
...
write_json(summary, summary_path)`),
  ...
  buildTable(
    ...
    [
      ...
      ["cluster-submit", "Submit the generated sbatch files (local/dev convenience; --dry_run prints planned submissions). Not used on TC1, where head-node Python is disallowed — TC1 jobs are submitted with direct sbatch (§5, TC1 runbook)."],
...
const ch5 = [
  ...
  buildTable(
    ...
    [
      ...
      ["pytest", "(installed via requirements.txt; 382 tests passing)"],
    ],
    [4200, 5160],
  ),
...
const ch6 = [
  ...
  ...Figure("asr_forest.png", "Quantization effect on harmful compliance at the 512-token reference budget: per-pair judge ΔASR (4-bit − fp16) with paired-bootstrap 95% confidence intervals. Filled markers denote intervals that exclude zero; only Llama-3.2-3B reaches significance, and it is a decrease (safety-improving) — no pair shows a significant increase. Source: results_512/analysis/judge_agreement.json."),
  ...
  buildTable(
    ...
    [
      ["HarmBench ΔASR (judge)", "0.000", "+0.040", "−0.040  ★", "−0.020", "+0.020"],
      ["  HarmBench ΔASR (v2 proxy)", "−0.025", "+0.070", "−0.020", "+0.060", "0.000"],
      ["  HarmBench McNemar exact p", "1.000", "0.096", "0.021", "0.627", "0.424"],
      ["XSTest Δover-refusal", "−0.024", "−0.004", "+0.016", "0.000", "−0.048  ★"],
      ["MMLU Δaccuracy", "−0.090  ★", "−0.003", "−0.037", "−0.020", "−0.027"],
      ["Interpretation label (judge-primary)", "broad_degradation", "alignment_degradation", "capability_collapse_masquerading_as_safety", "alignment_improvement", "alignment_degradation"],
      ["Evidence status (two-layer)", "confirmed", "directional", "directional", "directional", "directional"],
    ],
  ...
  PJ("The headline finding for RQ5 is that, at the 512-token reference budget under the official classifier, NF4 quantization produces no significant harmful-compliance increase in any pair, and the only individually-significant ΔASR (Llama-3.2-3B, −0.040) is a decrease. The Qwen3-1.7B pair shows ΔASR at exactly zero paired with a large, significant capability loss (ΔMMLU −0.090); the Qwen3-4B pair shows a directional, non-significant +0.040 at preserved capability; the Llama-3.2-3B pair shows a small significant ASR decrease alongside a significant ARC loss (§6.4.1). The capability dimension is directionally consistent (all five pairs have negative ΔMMLU; the Qwen3-1.7B loss is the only MMLU-significant one, with further ARC-significant losses on Qwen3-4B and Llama-3.2-3B). On over-refusal, the only significant delta (Phi, −0.048) is a decrease and the previously-borderline Qwen3-1.7B decrease is now non-significant (McNemar p = 0.109). The composite picture is that NF4 quantization does not produce a significant or uniform safety regression in any family or at any scale; the robust, multiplicity-surviving signals are all capability losses. See §6.11 for the full discussion and §6.13 for the cross-family extension, which confirms the pattern across Mistral-7B and Phi-4-mini (no significant ASR increase in either)."),
...
,
  ...
  ...Figure("judge_vs_proxy.png", "Scorer validation. Left: HarmBench-classifier ASR versus the regex 'non-refusal' proxy, one marker per model. Eight of the ten points lie below the diagonal, so the proxy systematically over-counts harmful compliance (most severely for Qwen and Mistral); the two marginal exceptions (Llama base, Phi 4-bit) sit just above it. Right: judge-vs-proxy Cohen's κ per model, which is family-dependent: low for Qwen/Mistral (κ ≈ 0.25–0.59) and high for Llama/Phi (κ ≈ 0.67–0.84). Source: results_512/analysis/judge_agreement.json."),
...
const refs = [
  ...
  ...numberedList([
    ...
    "A. Kharinaev, V. Moskvoretskii, E. Shvetsov et al., “Investigating the impact of quantization methods on the safety and reliability of large language models,” IEEE Access, vol. 14, pp. 96771–96793, 2026, doi: 10.1109/ACCESS.2026.3703899. arXiv:2502.15799.",
...
const appendixA = [
  ...
  PJ("The full configs/tc1_512.yaml — the configuration of the primary 512-token study (D41) — is reproduced below for reference, with explanatory comments lightly abridged. The retired 128-token configuration (configs/tc1.yaml, max_new_tokens: 128, writing to results/) is identical apart from the generation budget and output roots and is retained in the repository for the §6.16 generation-length comparison. The mistral_7b and phi4_mini entries are the cross-family extension: they were run on TC1 on 2026-06-15 and their results are incorporated into Chapter 6 (Tables 6.1–6.3 and §6.13)."),
  ...Code(tc1Yaml),
];
...
const appendixD = [
  ...
  PJ("The verification suite comprises 382 automated tests across twenty-nine test files. The distribution is summarised in Table D.1; the test files are located under tests/ in the repository."),
  buildTable(
    ...
    [
      ...
      ["test_judge_validation.py", "17", "Judge-model validation: stub-backend scoring, sidecar redaction enforcement, ASR aggregation, raw-immutability, idempotency, the official-template regression check, judge-precision resolution, and full status accounting (error/skipped counted, never silently dropped)."],
      ["test_pipeline.py", "12", "Batched generation and orchestration helpers."],
      ["test_agent_harness.py", "16", "Agent harness: AGENTS/CLAUDE sync, project-log discipline, immutable-artifact discipline (mutation fails; partial local deletion fails; fresh-clone absence passes), stale-text/redaction scans with real-policy self-tests (retired-κ guards fire unless 128-scoped; a leaky results sidecar fails redaction), agent-start packet, and report-freshness (a report-source edit without a regenerated docx fails the gate; the committed policy keeps the source a trigger, not a satisfier)."],
      ...
      ["test_sensitivity.py", "11", "Multi-seed sensitivity config + aggregator (summarise_deltas) and sbatch generator, plus the v2-proxy reader preferring summary.v2.json over the v1 runtime summary.json (with fallback), and the per-pair alias-ordering guard (aliases match PAIRS[pid] = (base, 4bit))."],
      ...
      ["test_dashboard_data.py", "17", "Streamlit-free dashboard data layer (dashboard/data.py): results discovery, judge-primary ASR rebuild from results/analysis/multiple_comparisons.json via classify_pair_change with canonical judge-label preference, summary loading, the schema-validated config + sbatch emission for the add-model form, config-filename sanitisation, and the protected-evidence execution guard (subpaths, traversals, and case-variant/aliased spellings all refused; the Run page resolves every execution dir through a single guarded choke point)."],
      ["test_report_claims.py", "1", "Claim lock (D43): runs scripts/verify_report_claims.py — every load-bearing number in the canonical report AND thesis builders is asserted against the committed analysis artifacts (55+ checks; the gate prints its exact count on every run); the documents cannot drift from their evidence without failing the suite."],
      ["test_dashboard_theme.py", "8", "Dashboard presentation helpers (dashboard/theme.py): label formatting, interpretation/metric colour mapping, and delta-rendering utilities."],
      ["test_xstest_judge.py", "13", "XSTest 3-class refusal-judge sensitivity layer (T35/D45): label-parser variants and misparse resistance, the byte-pinned rubric regression, the API backend label mapping and prompt/response slot routing (faked client), and sidecar redaction."],
      ["test_xstest_human_audit.py", "12", "XSTest human-label gold-set tool (T36/D46): deterministic stratified draw, disagreement-fraction and exclusions, double-weighted priority pairs, Cohen's kappa (incl. degenerate), precision/recall/F1 and confusion counts, outcome-letter read-off, and the frozen-rubric guard (HTML and source scanned for retired wording)."],
      ["test_parser_strict.py", "7", "Strict MMLU/ARC parser and rescore (T38): strict tier behaviour, the strict regexes as a literal restriction of the primary cascade, num-choices inference, raw-immutability + redacted sidecar emission, and dry-run-writes-nothing."],
      ["Total", "382", "Twenty-nine files; all passing on the current commit."],
    ],
    [3000, 1200, 5160],
  ),
...
const doc = new Document({
  ...
  features: { updateFields: true },  // prompt Word/LibreOffice to populate the TOC field on open
  styles: {
...
const OUTPUT = "/Users/tanueihorng/fyp_quant/docs/FYP_Report_2026-07-01_v5.docx";
