...
const appendixG = [
  ...
  buildTable(
    ...
    [
      ["2026-07-11 16:00", "FYP_Report_2026-07-01_v5.docx (current)", "Independent-audit remediation pass. Removed the internal future-session handoff content from Appendix H and reframed it as a scoring-methodology correction record, so no agent instructions, internal task identifiers, or cluster-operational steps remain in the deliverable. Added honest disclosures surfaced by the audit: the historical run artifacts predate resolved-revision / package-version / dataset-fingerprint capture in each record (the checkpoint revisions are pinned in the committed configs, and the exact environment is now a documented reproducibility caveat rather than a machine-recorded field); the INT8 precision-comparison configs now pin the same model revisions as the NF4 study; the HarmBench judge model and tokenizer revision is now pinnable and recorded with the judge output; XSTest over-refusal is scored by a regex proxy pending an independent-judge or blind human-label validation; and the HarmBench ASR statistical power (per-pair minimum detectable effects of about 0.04-0.09 at 200 items) and the three-of-five multi-seed coverage are stated as explicit bounds on the small-effect and cross-family conclusions. No result numbers changed."],
