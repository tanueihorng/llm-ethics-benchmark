...
const ch4 = [
  ...
  PJ("Generation itself uses torch.no_grad and a do_sample=False decode path under temperature=0.0. Outputs are post-processed to strip the chat-template prefix and any trailing whitespace before being returned to the calling pipeline."),
...
const refs = [
  ...
  ...numberedList([
    ...
    "R. J. Connor, “Sample size for testing differences in proportions for the paired-sample design,” Biometrics, vol. 43, no. 1, pp. 207–211, 1987.",
...
const appendixH = [
  ...
  PJ("The classifier has since been cross-checked three times: by an independent frontier-model second judge (gpt-4o, §6.12 Result 4), against human labels on a stratified subset (§6.12 Result 5), and by an open-weight guard model (Llama-Guard-3-8B, revision-pinned; §6.12), whose revision-pinned, locally rerunnable re-scoring — unlike the versioned API judge — agrees with the classifier at κ 0.36–0.92 across the ten base/NF4 aliases and finds every per-pair ΔASR non-significant, with the construct caveat that it answers a looser safety question than the behavior-specific classifier. All three run through the pluggable judge backend in `ethical_benchmark/judges/validation.py` and the agreement analysis in `scripts/judge_agreement.py`; the primary scorer is unaffected either way."),
];
