...
const ch1 = [
  ...
  ...numberedList([
    ...
    "An open, reproducible benchmarking framework (approximately 8,900 lines of production Python and 4,600 lines of automated tests; 339 tests across twenty-six files) with batched chat-templated generation, per-prompt audit logging, immutable hash-pinned raw artefacts, redacted score sidecars, and resumable per-model SLURM matrix jobs for the NTU TC1 cluster.",
...
const ch3 = [
  ...
  PJ("The independent variables are quantization state (baseline versus four-bit), model scale (two billion versus four billion parameters within the Qwen family), and model family (Qwen versus Llama in the primary design, extended to Mistral and Phi in the cross-family replication of §6.13). The dependent variables are harmful compliance, over-refusal on benign prompts, and general capability, each measured by a corresponding benchmark."),

  H2("3.2 Model Selection"),
  PJ("Three model pairs form the primary controlled design. The Qwen pairs at 1.7 billion and four billion parameters [16] serve as the primary within-family comparison and provide both a quantization axis and a scale axis. The Llama 3.2 3B pair [17] sits between them by parameter count and serves as a cross-family robustness check. Table 3.1 summarises this primary matrix; a cross-family extension later adds two further matched pairs (Mistral-7B and Phi-4-mini, introduced with the experimental setup in Chapter 5 and analysed in §6.13), taking the study to five pairs across four families."),
  buildTable(
...
const ch5 = [
  ...
  buildTable(
    ...
    [
      ...
      ["pytest", "(installed via requirements.txt; 339 tests passing)"],
    ],
    [4200, 5160],
  ),
...
const ch6 = [
  ...
  buildTable(
    ...
    [
      ...
      ["qwen_2b", "XSTest over-refusal", "0.056", "0.028", "−0.028 [−0.052, −0.004]", "borderline"],
      ...
      ["qwen_4b", "XSTest over-refusal", "0.032", "0.032", "0.000 [−0.016, +0.016]", "no"],
      ...
      ["llama_3_2_3b", "XSTest over-refusal", "0.036", "0.052", "+0.016 [−0.008, +0.040]", "no"],
      ...
      ["mistral_7b", "  HarmBench non-refusal (v2 proxy)", "0.825", "0.890", "+0.065 [+0.020, +0.110]", "(proxy)"],
      ["mistral_7b", "XSTest over-refusal", "0.004", "0.008", "+0.004 [−0.008, +0.016]", "no"],
      ...
      ["phi4_mini", "XSTest over-refusal", "0.128", "0.084", "−0.044 [−0.072, −0.016]", "yes"],
...
,
  ...
  PJ("The Qwen3-1.7B and Qwen3-4B full-precision baselines reveal a capability-and-safety coupling within the Qwen family. At full precision (HarmBench under the official classifier at the 512-token reference budget), scaling from 1.7B to 4B parameters produces: HarmBench ASR 0.255 → 0.115 (−14.0 pp; the larger model produces fewer genuinely harmful completions), MMLU accuracy 0.643 → 0.747 (+10.4 pp; substantially more capable), and XSTest over-refusal 0.056 → 0.032 (−2.4 pp; fewer false refusals on benign prompts). Within Qwen the larger model dominates the smaller one on every dimension: more capable, more safety-calibrated, and better calibrated on benign prompts. (Under the v2 non-refusal proxy the baseline ASR gap looks larger, reflecting the proxy's over-counting; the genuine harmful-compliance gap is the 14.0 pp classifier figure.) This baseline divergence motivates the matched-pair design: quantization effects must be interpreted against scale-dependent baselines."),
  ...
  buildTable(
    ...
    [
      ...
      ["mistral_7b_base", "0.825", "0.585", "68.0%", "0.28"],
...
const refs = [
  ...
  ...numberedList([
    ...
    "A. Kharinaev, V. Moskvoretskii, E. Shvetsov et al., “Investigating the impact of quantization methods on the safety and reliability of large language models,” IEEE Access, 2025. arXiv:2502.15799.",
...
const appendixD = [
  ...
  PJ("The verification suite comprises 339 automated tests across twenty-six test files. The distribution is summarised in Table D.1; the test files are located under tests/ in the repository."),
  buildTable(
    ...
    [
      ...
      ["Total", "339", "Twenty-six files; all passing on the current commit."],
    ],
    [3000, 1200, 5160],
  ),
...
│   └── metrics/
├── tests/                    (339 tests across 26 files)
...
const appendixG = [
  ...
  buildTable(
    ...
    [
      ...
      ["2026-06-06 16:00", "FYP_Report_2026-05-27.docx", "Doc-consistency follow-ups to the D16 judge-primary promotion. Rewrote Chapter 7 §7.1 (scoring determinism now describes the judge classifier, not regex-eliminates-judge-variance) and §7.4 (renamed \"Scorer Choice and Remaining Validity Threat\" — judge is primary, remaining threat is the absence of a second independent judge). Rewrote Appendix H §H.2–§H.5 to the post-judge state (the v2 Qwen 4B figure flagged as superseded; judge sidecars are the committed primary scorer; next steps are push/T1/T3/T15 plus an optional second judge, not an outstanding judge run). Updated the cover-page revision line to the judge-primary D16 wording. No numbers changed; this is a consistency pass so the .docx no longer contains pre-judge framing."],
...
const appendixH = [
  ...
  PJ("The workflow was verified in five ways. First, the full test suite passes with 339 tests, including v2 refusal-parser regression tests, the analysis sidecar-selection test, the capability-guard test for the refined interpretation rule, and judge-validation tests for redaction, raw immutability, idempotency, ASR aggregation, the official HarmBench classifier prompt template, and judge-precision resolution. Second, the judge jobs ran in full precision on a 32 GB V100 with zero parse errors throughout (512-token primary study: job 61524, 2 000 generations plus 1 000 INT8; 128-token comparison: jobs 61047/61134, 2 000 generations). Third, `scripts/judge_agreement.py` reproduces the judge-vs-v2 agreement and the judge-primary pair labels deterministically from the committed sidecars. Fourth, the original `raw.jsonl` and `summary.json` files were confirmed unchanged by every post-hoc step (rescore and judge). Fifth, all judge sidecars were checked for redaction: they contain prompt IDs, boolean labels, and run metadata only, with no prompt, behaviour, or response text."),

  H2("H.5 Where the next session should continue"),
  PJ("The HarmBench judge validation is complete at both budgets (128-token: jobs 61047/61134; 512-token primary: job 61524 plus the INT8 follow-up) and the official classifier is the primary HarmBench scorer (decision D16); §6.12 carries the results. There is no pending scoring work. Future agents should start from the repo-native handoff in `docs/HANDOFF.md`, then verify live Git state with `git status -sb`. The remaining non-technical items are: send the supervisor progress note (T1); record the exact storage quota via `MyTCinfo` (T3, hardware already confirmed as a 32 GB Tesla V100); and prepare the final submission (T15)."),
